...
`TODO_KEY` is not set, so the digest was NOT fetched. You are working blind on
board state.
...
To fix: set `TODO_KEY` as an environment variable on this Claude Code
environment. It must match the `TODO_KEY` the API runs with on Railway,
character for character, or the read comes back 401.
...
BODY=$(curl -sS --fail-with-body -m 20 \
  -H "Authorization: Bearer $TODO_KEY" \
  "$BASE/api/command/digest" 2>&1)
RC=$?
...
if [ $RC -ne 0 ]; then
  ...
  echo ""
  echo "Tried \`$BASE/api/command/digest\` and it failed. You have NO live board"
...
echo ""
echo "### COMMAND CENTER DIGEST (fetched at session start)"
echo ""
