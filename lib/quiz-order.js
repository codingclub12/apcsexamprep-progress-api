...
// Pick n distinct random elements from arr, preserving nothing about order (the
// caller shuffles anyway). n <= 0 or n >= arr.length returns the full array. Used
// for N-of-M: select which questions to serve before shuffling them.
function sample(arr, n) {
  if (!Number.isInteger(n) || n <= 0 || n >= arr.length) return arr.slice();
...
module.exports = { buildOrder, readOrder, sample };
