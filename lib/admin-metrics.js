...
// Headline metric keys. Every value here is snapshotted daily and returned with
// 24h / 7d deltas, so the dashboard can show change on any tile. Keep this list
// and the values object built in computeSummary in sync; the snapshot writer
// iterates it, and a value that is present here but missing from the object is
// snapshotted as 0 (its delta then reads against 0, not a dash).
const SNAPSHOT_METRICS = [
  // Raw top-line (all classes, unfiltered)
  'raw_classes',
  'raw_students',
  'raw_teachers',
  'raw_completions',
...
`);
// Raw counts for top-line deltas, defined to match /api/admin/overview so a tile
// value and its delta never disagree (teacher count is all rows, not just owners).
const stmtAttemptCount = db.prepare('SELECT COUNT(*) AS n FROM attempts');
const stmtTeacherCount = db.prepare('SELECT COUNT(*) AS n FROM teachers');
...
function computeSummary() {
  ...
  // Raw top-line (all classes, unfiltered) for its own deltas.
  let rawStudents = 0, rawCompletions = 0;

  for (const c of classes) {
    const bucket = classifyClass(c);
    bucketCounts[bucket] += 1;
    rawStudents += c.student_count;
    rawCompletions += c.completions;
    if (bucket === 'EXTERNAL') {
      external.push(c);
    } else if (bucket === 'SOLO') {
      addInto(solo, c);
  ...
  // Deltas for every headline metric (also writes today's baseline). Computed
  // last so it can include the monetization and engagement values above.
  const deltas = snapshotAndDelta({
    raw_classes: classes.length,
    raw_students: rawStudents,
    raw_teachers: stmtTeacherCount.get().n,
    raw_completions: rawCompletions,
    raw_attempts: stmtAttemptCount.get().n,
    ...
    premium_teachers: monetization.premium.teachers,
    premium_classes: monetization.premium.classes,
    premium_students: monetization.premium.students,
    free_classes: monetization.free.classes,
    free_students: monetization.free.students,
    sessions_total: engagement.sessions_total,
    active_minutes_total: engagement.active_minutes_total,
  });
