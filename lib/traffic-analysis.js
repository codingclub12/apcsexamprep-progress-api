...
function overview({ days = 90 } = {}) {
  const span = Math.max(14, Math.min(400, Number(days) || 90));
  // Two spans, so the longest window can be compared against the one before it.
  const lookback = Math.min(400, span * 2);
...
module.exports = {
  series, movingAverage, compare, project, movers, rankings, inventory, overview,
  OVERVIEW_WINDOWS,
