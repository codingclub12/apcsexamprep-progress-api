...
function computeHealth() {
  ...
  //  Students are enrolled and the class is live, but none of the graded writers
  //  has ever recorded anything. Either nobody has done work yet, or the grading
  //  reporters are not wired on the pages they are using.
  //
  //  quiz_attempts has to be one of the writers checked. It is where every
  //  client-submitted quiz lands (POST /api/student/quiz, which is the path all
  //  Cybersecurity quiz pages use), so leaving it out raised this CRITICAL
  //  against classes whose quizzes were arriving and grading perfectly well.
  const noGraded = db.prepare(`
    ...
    WHERE c.id IN (${inReal})
      ...
      AND NOT EXISTS (SELECT 1 FROM quiz_attempts q JOIN students qs ON qs.id = q.student_id
  ...
  const visitNoScore = db.prepare(`
    ...
    WHERE p.completed = 1 AND ${VISIT_ROWS} AND p.class_id IN (${inReal})
      ...
      AND NOT EXISTS (SELECT 1 FROM quiz_attempts q WHERE q.student_id = p.student_id)
    GROUP BY p.class_id
    ORDER BY students DESC
  ...
  //
  //  ── WHY A DELIBERATE RESET IS EXCLUDED ───────────────────────────────────
  //   A teacher Reset produces this exact state ON PURPOSE. It nulls
  //   progress.score, stamps score_reset_at, and LEAVES the pre-reset rows in
  //   the ledger, because they are still the student's history and still appear
  //   in GET /api/student/history. Read without that in mind, every reset looks
  //   like a rollup that failed, and this check said so in as many words: "the
  //   work was captured but the teacher's gradebook will read blank for it."
  //   That is precisely what the teacher asked for.
  //
  //   The distinction is whether anything has been submitted SINCE the reset.
  //   Reset, then silence, is the intended state and is not reported. Reset,
  //   then a submission, then a still-null score is a genuine failure and still
  //   fires: the exclusion is scoped to the quiet case rather than to the column.
  //
  //   Before 2026-08-24 the Reset button on the teacher dashboard wrote nothing
  //   at all (it was preview-only), so this could not fire for that reason and
  //   the gap was invisible. Wiring the button made a false CRITICAL arrive on
  //   the next use, which is how it was found.
  const rollupGaps = db.prepare(`
    ...
    WHERE (p.id IS NULL OR p.score IS NULL)
      AND NOT (
        p.score_reset_at IS NOT NULL
        AND NOT EXISTS (
          SELECT 1 FROM score_events se
          WHERE se.student_id = e.student_id AND se.course = e.course
            AND se.unit = e.unit AND se.lesson = e.lesson
            AND se.activity_type = e.activity_type
            AND se.created_at > p.score_reset_at
        )
      )
    GROUP BY e.class_id
  ...
  // ── CHECK 8: attempts lost to the quiz answer-parse bug ───────────────────
  //  Until the parse fix, /api/quiz/submit accepted only a literal integer on
  //  chosen_index and silently dropped anything else, scoring every dropped
  //  answer wrong. The failure persisted `sel: null` for the question, so an
  //  affected attempt is identifiable exactly: it scored 0 and NOT ONE question
  //  carries a numeric selection. A genuinely wrong answer always records the
  //  index the student picked, so the two cannot be confused.
  //
  //  Caveat worth stating: a student who submitted without answering anything
  //  looks identical in storage. Real parse failures show up as whole activities
  //  failing across many students at once, so the per-activity grouping below is
  //  what separates the two.
  const lostRows = db.prepare(`
    SELECT q.course, q.unit, q.lesson, s.class_id,
           COUNT(*) AS attempts, COUNT(DISTINCT q.student_id) AS students,
           MIN(q.attempted_at) AS first_seen, MAX(q.attempted_at) AS last_seen
    FROM quiz_attempts q
    JOIN students s ON s.id = q.student_id
    WHERE s.class_id IN (${inReal})
      AND COALESCE(q.score, 0) = 0
      AND q.answers IS NOT NULL
      AND q.answers LIKE '%"sel":null%'
      AND q.answers NOT GLOB '*"sel":[0-9]*'
    GROUP BY q.course, q.unit, q.lesson, s.class_id
    ORDER BY attempts DESC
    LIMIT 200
  `).all(...realIds).map((r) => ({
    ...label(r.class_id),
    course: r.course, unit: r.unit, lesson: r.lesson,
    attempts: r.attempts, students: r.students,
    first_seen: r.first_seen, last_seen: r.last_seen,
  }));
  if (lostRows.length) {
    const totalAttempts = lostRows.reduce((n, r) => n + r.attempts, 0);
    const totalStudents = lostRows.reduce((n, r) => n + r.students, 0);
    add(CRITICAL, 'attempts_lost_to_parse_bug',
      'Submissions scored zero with no selection recorded',
      `${totalAttempts} attempt(s) across ${totalStudents} student record(s) scored 0 with not one question carrying a selection. ` +
      'That is the signature of the answer-parse bug: the server could not read which option was picked and marked every question wrong. ' +
      'The picks were never stored, so these cannot be rescored. Clear the attempts and let the students retake, or disregard the scores.',
      lostRows);
  }

  // ── CHECK 9: the two graded pipelines disagree ────────────────────────────
