'use strict';
const { unitTestLinks, lowerFirst } = require('./csp-unit-tests');
const { renderStimulus } = require('./csp-exercise-stimulus');
// ─────────────────────────────────────────────────────────────────────────────
...
//      available on the site exercise." Part A stays paper work, EXCEPT its
//      log table when the source entry carries one: a graded check derived
//      from the key asks about that log's rows, so the page must show it. See
//      lib/csp-exercise-stimulus.js.
//
...
function renderExercise(handle, check) {
  ...
  // A null check is the MIRROR-ONLY page: the handout reproduced online with no
  // graded half, because nobody has authored its check questions against the
  // answer key yet. That is a real state, not a degraded one, and the page says
  // so rather than showing a score bar that counts nothing.
  const graded = !!(check && check.questions && check.questions.length);
...
${prompts}
  ...
  // ── the graded check ──────────────────────────────────────────────────────
  const n = graded ? check.questions.length : 0;
  const items = !graded ? '' : check.questions.map((q, i) => {
    const qid = `chk-${i + 1}`;
    ...
    return `        <div class="mcq-item" data-activity="${act}" data-item="${qid}">
          <div class="mcq-q-num">Question ${i + 1} of ${n}</div>
...
${renderStimulus(src, id)}  <h2>${esc(mirrorHeading)}<span class="tag local">Not recorded</span></h2>
  <p>${esc(mirrorNote)} <span class="saved" id="draft-note"></span></p>
${mirror}
${graded ? `  <h2>Graded check<span class="tag graded">Recorded</span></h2>
  ...
  <div class="item">
${items}
  </div>` : `  <h2>No graded check on this page yet<span class="tag local">Not recorded</span></h2>
  <div class="note">
    <p class="label">About the line in your handout</p>
    <p>Your handout says this exercise is available online and auto-graded. The exercise IS here, and it is the same work in the same order, but the auto-graded half is not written for this topic yet. Nothing on this page is scored or sent to your teacher.</p>
    <p>Hand this in the way your teacher asked for it. Topic 1.1 has its graded check today, and the rest follow.</p>
  </div>`}
  <div class="nextnav">
    <a href="/pages/${lessonHandle}">Rework the Topic ${src.topic} lesson</a>
    ${unitTestLinks(bi, { esc, text: (l) => `Big Idea ${bi} ${lowerFirst(l)}` })}
  </div>
</div>
${script(handle)}`;
  ...
  return {
    ...
    seoDescription: (graded
      ? `Online companion to the AP CSP Topic ${src.topic} Exercise ${src.exercise} handout, ${src.title}. Work the cases, then take the auto-graded check.`
      : `Online companion to the AP CSP Topic ${src.topic} Exercise ${src.exercise} handout, ${src.title}. Work the items here, with your writing saved in your own browser.`
    ).slice(0, 160),
  };
}

// Every check file that exists, merged into one handle -> check index. Adding a
// topic's checks is dropping a file into seed/csp-exercise-checks and nothing
// else: the directory IS the list, so a file that exists but was forgotten in a
// hand-maintained index cannot happen. Files are read in sorted order purely so
// the merge is deterministic.
function allChecks() {
  const dir = path.join(__dirname, '..', 'seed', 'csp-exercise-checks');
  const out = {};
...
// The 70 handles printed inside the student handouts, in source order. A page
// with authored checks renders its graded half; the rest render mirror-only.
// The full set is what the printed URLs need, so the default is all of them.
function allPages() {
  const checks = allChecks();
  return Object.keys(SOURCE).map((h) => renderExercise(h, checks[h] || null));
}

// Just the graded ones, which is what the denominator seed prices.
function gradedPages() {
  const checks = allChecks();
  return Object.keys(checks).map((h) => renderExercise(h, checks[h]));
}

module.exports = { allPages, gradedPages, allChecks, renderExercise, BY_TOPIC, SOURCE, esc };
