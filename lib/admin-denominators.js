...
//  THE SAME TRAP, A SECOND TIME, FOUND 2026-09-07.
//  A page that reports for itself has TWO writers, not one: its own named item,
//  and assets/apcs-score-reporter.js scraping the number it displays and posting
//  it under the reserved 'score' item. On AP Cyber 1.1 Exercise 1 that summed to
//  14 out of 14 for a 7 point exercise, and adopt would have authored 14 as the
//  official total. Same exclusion, same reason, and the rule is stated once in
//  scoring.js so this proposal and the gradebook cannot disagree about it.
const OBSERVED_FROM_EVENTS = `
  ...
  FROM (
    SELECT unit, lesson, activity_type, student_id, item, MAX(max_points) AS item_max,
      ${namedItemFlagSql('item', 'unit, lesson, activity_type, student_id')} AS has_named
    FROM score_events
