...
function unitIndex(course) {
  ...
  for (const [unitId, u] of Object.entries(cfg.units)) {
    ...
    if (u.exam && !idx.has(u.exam.lesson)) idx.set(u.exam.lesson, unitId);
  }
  return idx;
}
...
function coverage(course, opts) {
  ...
  if (cfg && cfg.units) {
    for (const [unitId, u] of Object.entries(cfg.units)) {
      ...
      if (u.exam) ensure(u.exam.lesson, 'exam', unitId);
    }
  }
...
module.exports = { coverage, adopt, compareLessonId, VISIT_ACTS };
