...
// The ArticleCreateInput minus blogId. This is the ONE function that decides
// what an article looks like on the wire, used by both the token-based
// publisher below and `blog.js emit`, which hands the same object to an agent
// driving the Shopify MCP connector. Two callers, one source of truth: the
// connector path must never be free to construct its own payload, because
// that reopens the transcription risk this whole design exists to close.
function articlePayload(post) {
  const m = post.meta;
  return {
    title: m.title,
    handle: m.handle,
...
function articleInput(post, blogId) {
  return { blogId, ...articlePayload(post) };
}
...
module.exports = { publish, admin, scopes, blogIds, liveHandles, articleInput, articlePayload };
