...
// Sizes that may run with no human before the PR. Tanner's rule, in his words:
// anything double-checked that "doesn't make major large multi page changes",
// or done overnight with a morning check. `l` and `xl` are the sizes that mean
// a large multi-file diff, so they stay off the list and stay hand-driven.
//
// This is a CEILING, not a preference. Widening it is a one-line edit here with
// history, which is the point: the list of what may run unattended should be a
// thing you can read in one line and argue with, not a condition spread across
// a dispatcher.
const AUTO_DISPATCH_MAX_SIZES = ['xs', 's', 'm'];
...
function autoDispatchEligibility(task, opts) {
  ...
  // Size ceiling, checked after the never-auto list so a money or migration task
  // is refused for THAT reason. "Touches pricing" is the more important thing to
  // tell someone than "has no estimate", and the existing suite asserts as much.
  //
  // An unset size counts as too big on purpose: "nobody estimated this" is not
  // evidence that it is small.
  const size = norm(task.size);
  if (!size) {
    return { eligible: false, reason: 'No size set. An unestimated task is not known to be small, so it is not dispatchable.' };
  }
  if (!AUTO_DISPATCH_MAX_SIZES.includes(size)) {
    return {
      eligible: false,
      reason: `size=${size} is a large multi-file change, and those stay hand-driven. Dispatchable sizes: ${AUTO_DISPATCH_MAX_SIZES.join(', ')}.`,
    };
  }

  return { eligible: true, reason: `Repo-reachable, open, unblocked, size=${size}, and off the never-auto-merge list.` };
}
...
module.exports = {
  ...
  SHOPIFY_PUSH_CEILING_BYTES, DEFAULT_TIME_WEIGHTING, NEVER_AUTO, AUTO_DISPATCH_MAX_SIZES,
};
