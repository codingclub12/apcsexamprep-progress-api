...
function listTeachers() {
  ...
  const teachers = stmtTeacherRows.all().map((t) => {
    ...
    const nScores = sc ? sc.n : 0;
    const nAttempts = at ? at.n : 0;
    const nVisits = vi ? vi.n : 0;
    const last = [vi && vi.last_ts, sc && sc.last_ts, at && at.last_ts].filter(Boolean).sort().pop() || null;
...
function teacherDetail(teacherId, opts) {
  ...
  const byClass = (sql) => mapBy(db.prepare(sql).all(...ids), 'class_id');
  ...
  const sco = byClass(`
    ...
    FROM score_events WHERE class_id IN (${ph}) GROUP BY class_id`);
  const att = byClass(`
    SELECT class_id, COUNT(*) AS n, COUNT(DISTINCT student_id) AS students,
  ...
  // Score events split by kind, so "are they using CFUs vs quizzes vs code" is
  // answerable per class. activity_type is 'cfu'/'quiz' for those paths and the
  // item name for code exercises, so anything else is bucketed as 'other'.
  const kindRows = db.prepare(`
    ...
    FROM score_events WHERE class_id IN (${ph}) GROUP BY class_id`).all(...ids);
  ...
  const classViews = classes.map((c) => {
    ...
    if (last && last < new Date(Date.now() - 14 * 864e5).toISOString().slice(0, 19).replace('T', ' ') && (nScores > 0 || nAttempts > 0)) {
  ...
  const scoreByStudent = mapBy(db.prepare(
    `SELECT student_id, COUNT(*) AS n, MAX(created_at) AS last_ts FROM score_events
  ...
  const roster = rosterRows.map((s) => {
    ...
    return {
      ...
      attempts: at ? at.n : 0,
      // The per-student version of the sync question.
      syncing: (sc ? sc.n : 0) + (at ? at.n : 0) > 0 ? true : ((vi ? vi.n : 0) > 0 ? false : null),
    };
  });
  ...
  const series = (sql) => {
    const m = {};
    for (const r of db.prepare(sql).all(...ids)) m[r.d] = r.n;
    return m;
  };
  ...
  const sDay = series(`SELECT DATE(created_at) d, COUNT(*) n FROM score_events
...
function featureUsage(v) {
  ...
  return [
    ...
    f('code', 'Code exercises graded', (v.code || 0) > 0, `${v.code || 0} events`),
