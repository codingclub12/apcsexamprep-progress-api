...
function analyze(text, opts) {
  ...
  return acceptRuns(hits);
}

// -- ISOLATED EXOTIC LEADS ARE REAL TEXT, NOT MOJIBAKE ------------------------
//  Found on 2026-09-03 by running this detector over the theme repo, where it
//  reported three of Shopify's own locale files as corrupt. They are not:
//
//    fi.schema.json   "Aakkosjarjestyksessa O-A"   O-diaeresis then an en dash
//    nb.schema.json   "Alfabetisk, A-A"            A-ring then an en dash
//
//  An O-diaeresis is U+00D6, inside the lead class. An en dash is U+2013, which
//  cp1252 maps to 0xB6, a continuation byte. So the pair is byte D6 96, which
//  is valid UTF-8 for U+0596, a Hebrew accent. The reversal is arithmetically
//  perfect and completely wrong: a Finnish sort-order label did not lose a
//  Hebrew combining accent. A capital letter followed by a dash is just text.
//
//  Two fixes were measured and rejected before this one:
//
//    restrict 2 byte leads to U+00C2 and U+00C3, which is what the previous
//      detector effectively did. It kills these three, and it also kills 12 of
//      the 120 generated cases: depth 2 corruption of an emoji is
//      C3-B0 C5-B8 C5-BD C2-AF, and the two middle pairs have exotic leads.
//      Losing depth 2 emoji recovery to fix a locale file is a bad trade.
//
//    reject a dash or curly quote as a continuation, which kills real
//      corruption of N-tilde, O-acute, O-diaeresis and their neighbours,
//      because those encode with exactly those second bytes.
//
//  What separates them is not the character, it is the COMPANY it keeps.
//  Mojibake is a whole-text transformation: it corrupts every non-ASCII
//  character it touches, so an exotic lead shows up INSIDE a run of other
//  corrupted characters, never alone in a sentence that is otherwise fine.
//
//  So a 2 byte candidate whose lead is outside U+00C2-U+00C3 is accepted only
//  if it abuts another accepted candidate. Runs still resolve completely,
//  because acceptance propagates along the chain until it stops changing.
//  Widths 3 and 4 are never restricted: their leads cannot be confused with
//  ordinary Latin text the way a capital O-diaeresis can.
const SAFE_LEAD_MIN = 0xC2;
const SAFE_LEAD_MAX = 0xC3;

function acceptRuns(hits) {
  if (hits.length < 2) {
    return hits.filter((h) => !isExotic(h));
  }
  const accepted = hits.map((h) => !isExotic(h));
  // Propagate to a fixpoint rather than in one sweep, so a chain of exotic
  // leads hanging off one safe anchor resolves all the way along.
  let changed = true;
  while (changed) {
    changed = false;
    for (let i = 0; i < hits.length; i++) {
      ...
      const prev = hits[i - 1];
      const next = hits[i + 1];
      const touchesPrev = prev && accepted[i - 1] && prev.index + prev.width === hits[i].index;
      const touchesNext = next && accepted[i + 1] && hits[i].index + hits[i].width === next.index;
      if (touchesPrev || touchesNext) { accepted[i] = true; changed = true; }
    }
  }
  return hits.filter((h, i) => accepted[i]);
}

function isExotic(h) {
  if (h.width !== 2) return false;
  const lead = h.chunk.codePointAt(0);
  return lead < SAFE_LEAD_MIN || lead > SAFE_LEAD_MAX;
}
...
module.exports = {
  analyze, repairOnce, repair, acceptRuns, isExotic,
