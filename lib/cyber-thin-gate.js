...
async function thinGate(chromium, exec, before, after, opts = {}) {
  ...
  //  The list lives in cyber-page-gate so the lesson pages and every other page
  //  shape in the unit check the same keys. It used to live here, where the only
  //  shapes it had ever seen were lessons.
  fail.push(...gate0.keysUnchanged(before, after));
  note.push('every graded key unchanged: MCQ, sequence, match, dtb, quiz ANSWERS, '
    + 'JSON keys, option values');
