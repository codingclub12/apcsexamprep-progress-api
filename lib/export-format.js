...
function canvasActivityColumns(course, courseConfig) {
  ...
  const cols = [];
  for (const unitId of Object.keys(courseConfig.units)) {
...
function buildCanvasActivityExport({ course, courseConfig, className, students, cellFor }) {
  const cols = canvasActivityColumns(course, courseConfig);

  const headers = ['Student', 'ID', 'SIS User ID', 'SIS Login ID', 'Section']
    .concat(cols.map((c) => c.header));
  const pointsRow = ['Points Possible', '', '', '', ''].concat(cols.map(() => String(UNIT_POINTS)));
  ...
  for (const s of students) {
    ...
    const grades = cols.map((c) => formatCell(cellFor(s.id, c.unit, c.lesson, c.activity), 'canvas'));
...
module.exports = {
  ...
  ACTIVITY_LABELS, canvasActivityColumns,
