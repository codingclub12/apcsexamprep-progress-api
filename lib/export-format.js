...
//  student_ref, which the teacher sets to the SIS Login ID Canvas already knows.
//  student_ref is free text and unvalidated, so anything that would corrupt the
//  CSV or silently mis-match is emitted as a blank instead.
...
function buildCanvasUnitExport({ course, courseConfig, className, students, cellFor }) {
  ...
  const headers = ['Student', 'SIS Login ID', 'Section']
  ...
  for (const s of students) {
    const sis = canvasSisLoginId(s.student_ref);
    if (sis) matchable++; else unmatchable.push(s.display_name);
    ...
    rows.push([canvasStudentName(s.display_name), sis, className || ''].concat(grades));
  }
...
module.exports = {
  ...
  canvasSisLoginId, canvasStudentName, buildCanvasUnitExport,
};
