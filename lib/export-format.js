...
// A unit may declare one exam or several (CSP Big Idea 3 sits its unit test in
// two parts). Imported rather than restated: a second copy of that rule is how
// two views drifted apart before.
...
function normalizeCell(rec) {
  ...
  if (rec.points != null) {
    ...
    if (p && p.possible > 0) {
      // priced: these are REAL marks, counted by the page. An export that needs
      // a points total (Schoology) can use them.
      return { kind: 'score', priced: true, pct: clampPct((p.earned / p.possible) * 100), earned: p.earned, possible: p.possible };
    }
  ...
  if (rec.score != null && rec.score !== '') {
    ...
    if (Number.isFinite(n)) {
      ...
      // priced: FALSE. earned/possible here are the percentage written as "n out
      // of 100", which is a rendering of the percent and not a count of marks.
      // Nobody assigned 100 points to this activity. A percentage export may use
      // it; an export that states a points total must not, or it publishes an
      // assignment worth 100 marks that exists on no page.
      return { kind: 'score', priced: false, pct, earned: pct, possible: 100 };
    }
  }
...
// The unit columns, as data. scope=unit emits one Canvas assignment per unit,
// and both the CSV header and the assignment in the course package have to be
// the same string or the import creates a second, empty assignment beside the
// one the teacher already has. Extracted so there is exactly one function that
// decides what a unit column is called.
function canvasUnitColumns(course, courseConfig, filter) {
  const prefix = CANVAS_COURSE_LABELS[course] || (courseConfig && courseConfig.label) || course;
  return selectedUnitIds(courseConfig, filter && filter.units).map((unitId) => ({
    unit: unitId,
    lesson: null,
    activity: null,
    header: `${prefix} ${shortUnit(unitId)}`,
    label: (courseConfig.units[unitId] && courseConfig.units[unitId].label) || shortUnit(unitId),
  }));
}
...
function canvasActivityColumns(course, courseConfig, filter) {
  ...
  for (const unitId of selectedUnitIds(courseConfig, filter && filter.units)) {
    ...
    if (activityIncluded(include, 'exam')) {
      for (const ex of examsOf(u)) {
        cols.push({
          unit: unitId, lesson: ex.lesson, activity: 'exam',
          header: unique(`${prefix} ${shortUnit(unitId)} ${ex.label || 'Unit Exam'}`),
        });
      }
    }
  }
...
function buildCanvasUnitExport({ course, courseConfig, className, students, cellFor, filter }) {
  const include = filter && filter.include;
  // Through canvasUnitColumns, never rebuilt inline: the course package names its
  // unit assignments from the same call, and a Canvas gradebook import matches on
  // the assignment NAME. Two copies of this string is two assignments.
  const cols = canvasUnitColumns(course, courseConfig, filter);
  const unitIds = cols.map((c) => c.unit);
  ...
  for (const s of students) {
    ...
    const unitPcts = unitIds.map((unitId) => {
      ...
      if (u.exam && activityIncluded(include, 'exam')) {
        for (const ex of examsOf(u)) cells.push(normalizeCell(cellFor(s.id, unitId, ex.lesson, 'exam')));
      }
...
// ── Schoology export ─────────────────────────────────────────────────────────
//  Schoology imports a gradebook by MAPPING columns: the teacher uploads a CSV,
//  picks the column that identifies students, then maps each score column onto
//  an assignment that already exists in the course. So unlike Canvas, the header
//  text is not load bearing, and there is no fixed identity schema to satisfy.
//
//  What IS load bearing is the number in the cell. A Schoology assignment has a
//  max points, and the imported score is raw marks against it. So this export
//  states marks, not percentages: "7" against an assignment created out of 8.
//  That is the whole difference from the Canvas export, which authors every
//  assignment out of 100 and posts a percentage into it.
//
//  A column is therefore only exportable if its marks are REAL. A percent-only
//  column has no marks and no total: nobody assigned points to that activity, so
//  there is no max for the teacher to create the assignment with. Those columns
//  are dropped from the file and listed in preflight.skipped, because a silently
//  missing column is how a teacher discovers half a gradebook did not import.
//
//  Where students were served different totals for the same activity, the column
//  total is the LARGEST seen, and the disagreement is reported. Schoology has one
//  max per assignment, so some single number has to be chosen; choosing the
//  largest never inflates a student's score above what they earned.
//
//  A blank cell is ungraded and must stay blank. Writing 0 for a completed but
//  unscored activity imports a fabricated failure, which is the phantom-zero bug
//  the ungraded-fallout suite exists to keep dead.
function buildSchoologyExport({ course, courseConfig, className, students, cellFor, filter }) {
  const cols = canvasActivityColumns(course, courseConfig, filter);

  // First pass: how many marks is each column actually out of? Only priced
  // cells count. A column nobody has assigned points to gets no total, and so
  // no assignment.
  ...
  for (const c of cols) {
    ...
    for (const s of students) {
      const cell = normalizeCell(cellFor(s.id, c.unit, c.lesson, c.activity));
      if (cell.kind === 'score' && cell.priced && cell.possible > 0) seen.add(cell.possible);
    }
    if (!seen.size) continue;
    const list = [...seen].sort((a, b) => b - a);
    totals.set(c.header, list[0]);
    if (list.length > 1) mixed.push({ column: c.header, totals: list });
  }

  const keep = cols.filter((c) => totals.has(c.header));
  const skipped = cols.filter((c) => !totals.has(c.header)).map((c) => c.header);

  const headers = ['First Name', 'Last Name', 'Unique ID']
    .concat(keep.map((c) => `${c.header} (out of ${totals.get(c.header)})`));
  const pointsRow = ['Points Possible', '', '']
    .concat(keep.map((c) => String(totals.get(c.header))));

  const rows = [];
  let identified = 0;
  const unidentified = [];
  for (const s of students) {
    // The RAW display name, not canvasStudentName: that one emits "Last, First"
    // for Canvas's single Student column, which is backwards for two columns.
    const name = String(s.display_name == null ? '' : s.display_name).trim();
    const sp = name.split(/\s+/).filter(Boolean);
    const first = sp.length > 1 ? sp.slice(0, -1).join(' ') : name;
    const last = sp.length > 1 ? sp[sp.length - 1] : '';
    const ref = s.student_ref || '';
    if (ref) identified++; else unidentified.push(s.display_name);

    const grades = keep.map((c) => {
      const cell = normalizeCell(cellFor(s.id, c.unit, c.lesson, c.activity));
      // Only real marks are exported. A percent-only cell in a priced column
      // (one student reported marks, another only a percent) has no marks to
      // state, so it stays blank rather than being rescaled onto the column
      // total, which would invent a score the student never earned.
      if (cell.kind !== 'score' || !cell.priced || cell.earned == null) return '';
      return String(round2(cell.earned));
    });
    rows.push([first, last, ref].concat(grades));
  }
...
module.exports = {
  ...
  ACTIVITY_LABELS, ACTIVITY_BUCKETS, INCLUDE_BUCKETS, activityIncluded, selectedUnitIds,
  canvasActivityColumns, canvasUnitColumns,
  buildCanvasUnitExport, buildCanvasActivityExport, buildSchoologyExport,
