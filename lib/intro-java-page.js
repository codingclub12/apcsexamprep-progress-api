...
// Screenshots are served from the theme's asset CDN. The build script rewrites
// this if the assets land somewhere else; it is one constant on purpose.
const SHOT_BASE = `${SITE}/cdn/shop/files`;
...
<img src="${escAttr(SHOT_BASE + '/' + s.shot.src.split('/').join('-'))}"
     alt="${escAttr(s.shot.alt)}" loading="lazy" width="960" height="600">
</figure>`;
    } else if (s.shot) {
...
function renderJsonLd(l, handle, unitLabel) {
  ...
  const crumbs = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Intro to Java with Greenfoot', item: `${SITE}/pages/intro-java` },
      { '@type': 'ListItem', position: 2, name: unitLabel, item: `${SITE}/pages/intro-java-unit-1` },
...
function renderLesson(lesson, opts) {
  ...
  const body = [
    ...
    `<p class="ij-meta">${esc(unitLabel)} &middot; Lesson ${esc(lesson.lesson)}`
    ...
    renderQuestions(lesson.cfus, 'cfu', lesson.lesson, 'Check your understanding',
      'Answer these before moving on. They are marked instantly and you can retry.'),
    renderGap(lesson),
    renderQuestions(lesson.quiz, 'quiz', lesson.lesson, 'Lesson quiz',
      'This one counts towards your progress. Take it when the section above makes sense.'),
    renderRecap(lesson),
    renderStuck(lesson, helpIndex),
    renderNav(lesson, o.prev, o.next),
...
module.exports = { renderLesson, handleFor, publicQuestion, publicHole, esc, escAttr, WRAP, SITE };
