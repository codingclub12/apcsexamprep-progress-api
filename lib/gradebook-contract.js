...
const { LESSON_SCORE_ITEM, REPORTER_TOTAL_ITEM, namedItemFlagSql, keepItemSql } = require('../scoring');
const { resolveAliasGate, pickGateRow, rowScope } = require('./activity-gate');

const CONTRACT_VERSION = 1;

// There is deliberately NO provisional weight here any more. A column with no
// authored total used to be priced out of 100 so every graded cell would show a
// pair. The percentage that produced was correct and the weight was not: 100
// outweighs a real 5 mark quiz twenty to one, and it put denominators on screen
// (and in row totals) that appear on no page. A denominator is the points
// possible on that page. Unknown stays unknown, and shows up in
// integrity.missing_denominators as work to do.
...
const ACTIVITY_MAP = {
  ...
  lab: ['lab', null],
  // Deliberate extension, 2026-09-04. The simulated-shell labs in config/labs/
  // declare 'terminal-lab' so their manifest denominator stops colliding with
  // the per-lesson 'lab' widget on the same lesson id. Canonically they are
  // still labs and a teacher should see them as such, so this maps to 'lab'
  // with no ordinal: a terminal lab is not "lab 2". What the rename buys is a
  // distinct KEY, since denominatorMap and attempt-rollup both key on the
  // native name. Before it, grading the Topic 1.2 terminal lab silently
  // rewrote that lesson's lab denominator from 30 points to 8.
  'terminal-lab': ['lab', null],
  quiz: ['quiz', null],
  exam: ['exam', null],
  'case-file': ['project', null],
  // Deliberate extension, 2026-08-19. AP Networking's unit documentation records
  // and its team task are native 'project': work a teacher scores by hand rather
  // than work a page reports. Without this line they fall through to the
  // practice default and show up in integrity.unmapped_activities, which is the
  // map doing its job and telling us to come here.
  project: ['project', null],
  // Deliberate extension, 2026-09-01. AP CSA's debugging drills record as native
  // 'debug'. They were falling through to the practice default, which is the
  // forgiving side of the line and, more to the point, the side that means
  // "nobody has decided yet". They are authored alongside the exercises, carry a
  // task list and starter code, and have right and wrong outcomes, so they are
  // exercises. No ordinal: a debug drill is not "exercise 4".
  //
  // Safe on both counts that could have bitten. Retry policy keys on the NATIVE
  // name against a closed ['quiz','exam'] assessment list, so this does not make
  // a debug drill one-shot. And item_key is `${unit}/${ref}/${native}`, so a
  // lesson holding both an 'exercise' and a 'debug' still gets two columns
  // rather than one merged cell.
  debug: ['exercise', null],
};
...
// One canonical activity can arrive under two native names on the SAME lesson,
// and then the canonical label is drawn twice. Every cyber lesson carries a
// 'lab' column from utils.js and a 'terminal-lab' column from the manifest, and
// both rendered as "1.2 Lab": two identical headers, two padlocks, one of them
// over an activity that does not exist on most lessons. A teacher cannot act on
// a column they cannot name. The native override is the exception, not the rule:
// add one only where two natives share a canonical activity on one lesson.
const NATIVE_LABEL = { 'terminal-lab': 'Terminal Lab' };

function round1(n) { return Math.round(n * 10) / 10; }
function round2(n) { return Math.round(n * 100) / 100; }

//  THE GRADE RULE, stated once, because it is the one piece of arithmetic two
//  views kept restating and drifting on. Points over points, and null rather
//  than zero when nothing was attempted: a student who has started nothing is
//  ungraded rather than failing, and a column nobody has priced has no average
//  rather than an average of zero.
//
//  It returns the RAW percentage and rounds nothing, so each view keeps its own
//  precision (this file to one decimal, the operator view to whole percents)
//  without either one owning the rule. A caller must test for null before
//  rounding: round1(null) is 0, which is the answer this exists to refuse.
//
//  Exported for lib/admin-gradebook.js, which is the second builder over this
//  same data. Sharing pointsFromRatio already stopped the two rounding points
//  differently; this stops them dividing differently.
function gradePct(earned, graded) {
  return graded > 0 ? (earned / graded) * 100 : null;
}
...
const classStmt = db.prepare(`
  SELECT id, class_code, class_name, course, mastery_threshold, retry_allowed,
         retry_mode, games_graded, teacher_id, quiz_lock_default
  FROM classes WHERE id = ? OR class_code = ?
`);

// ── AVAILABILITY, alongside the grades ───────────────────────────────────────
//  A gradebook that shows what a student scored but not whether they can reach
//  the work is half an answer: "why has nobody done the Unit 3 quiz" and "the
//  Unit 3 quiz is locked" are the same fact, and a teacher should not have to
//  hold two screens open to join them.
//
//  Two class-scoped reads, not one per item. Both resolve through
//  lib/activity-gate.js, the same ladder the render path and the submit path
//  walk, so a padlock drawn here can never disagree with what a student gets.
const gateRowsStmt = db.prepare(`
  SELECT unit, lesson, activity_type, open, updated_at
  FROM activity_gates WHERE class_id = ? AND course = ?
`);

//  WHICH LOCKS ARE REAL. A lock only bites where the SERVER hands out the
//  questions, which is quiz_bank. An activity whose questions are baked into
//  the Shopify page body cannot be locked at all: the browser has the whole
//  instrument before any gate runs, and View Source defeats it. Every AP
//  Cybersecurity Unit 1 quiz was in that state when gating was built, so this
//  is the common case rather than the edge one. The contract therefore reports
//  enforceability beside the lock, and a UI that draws the toggle without it is
//  promising a teacher something the server cannot deliver. See
//  docs/quiz-locking.md.
const bankLocationsStmt = db.prepare(`
  SELECT unit, lesson, activity_type FROM quiz_bank
  WHERE course = ? AND active = 1
  GROUP BY unit, lesson, activity_type
`);

//  quiz_bank is not the only server render path, and treating it as the only one
//  was wrong in a way a teacher hit on 2026-09-07: labs are served from
//  config/labs by routes/labs.js, so the server DOES control their delivery and
//  can withhold it. This reported every lab as unenforceable, which said "that
//  padlock does nothing" about a lock that now does something.
//
//  The lesson generalizes past labs. Enforceability is "does the SERVER hand this
//  out", and every server-side delivery path has to be listed here or the
//  gradebook lies about it. Adding a third path means adding it here too.
//  EVERY alias, and the aliases stay GROUPED rather than flattened into names.
//  A lab spec calls itself 'terminal-lab' and utils.js lists 'lab' in every cyber
//  unit's activities, so a cyber lesson carries BOTH columns and a teacher sees
//  two padlocks over one lab. Two things follow, and they are the two bugs a
//  teacher reported on 2026-09-09:
//
//    enforceability  reporting only the spec's own name marked the 'lab' column
//                    unenforceable, which draws "this activity keeps its
//                    questions in the page, so students can still reach them"
//                    over a lock routes/labs.js does enforce. A teacher told a
//                    padlock does nothing clicks it freely.
//    availability    routes/labs.js resolves BOTH names and takes the narrowest
//                    answer, so a row on either one decides what the student
//                    gets. This file resolved ONE, so the board could draw a
//                    column open while the student was refused, which is the
//                    complaint verbatim: "But I have not locked them?"
//
//  So the group is the unit of truth here. Both readers below are built from it,
//  which is what stops the enforceability list and the availability ladder
//  drifting apart the way the two ladders just did.
//
//  Every SERVER-DELIVERED activity belongs in here or the gradebook lies about
//  it: labs from config/labs via routes/labs.js, analysis activities from
//  config/analysis via routes/analysis.js. Adding a third delivery path means
//  adding it here too.
const specAliasGroups = () => {
  const groups = [];
  const add = (modPath) => {
    //  A missing or unreadable config must not blank a gradebook, so each source
    //  fails to nothing on its own rather than taking the other down with it.
    try {
      const mod = require(modPath);
      for (const sp of mod.all()) {
        groups.push({
          course: sp.course, unit: sp.unit, lesson: sp.lesson_id,
          names: mod.aliases(sp),
        });
      }
    } catch (e) { /* no specs from this source */ }
  };
  add('./lab-spec');
  add('./analysis-spec');
  return groups;
...
//  The flat form the enforceability set wants: one entry per (location, name).
const serverDeliveredLocations = (groups) => {
  const out = [];
  for (const g of groups) {
    for (const act of g.names) {
      out.push({ course: g.course, unit: g.unit, lesson: g.lesson, activity_type: act });
    }
  }
  return out;
};

//  `${unit}|${lesson}|${native}` -> every name a gate row may use for that same
//  activity. A column with no entry resolves under its own name alone, which is
//  every column in every course that does not have two spellings.
const aliasIndexFor = (groups, course) => {
  const idx = new Map();
  for (const g of groups) {
    if (g.course !== course || !g.unit || !g.lesson) continue;
    for (const act of g.names) idx.set(`${g.unit}|${g.lesson}|${act}`, g.names);
  }
  return idx;
};
...
//
// The 'score' carrier is dropped on the same reasoning, but only where the page
// also reports its own named items: it is assets/apcs-score-reporter.js
// scraping the number the page already displays, so where a page reports for
// itself the two rows are one run counted twice. The rule and the measured case
// are in scoring.js beside REPORTER_TOTAL_ITEM; the fragments are imported
// rather than restated so this pass and progress.score cannot disagree.
...
function normalizeGrid(course, manifestRows, gamesGraded) {
  ...
  for (const u of unitKeys) {
    ...
    for (const ex of examsOf(cu)) ensureLesson(u, ex.lesson, 901, ex.label);
  }
  ...
  const addItem = (unit, ref, native, opts) => {
    ...
    if (!items.has(key)) {
      ...
      items.set(key, {
        ...
        label: `${ref} ${NATIVE_LABEL[native] || ABBR[c.activity] || c.activity}${c.ordinal ? c.ordinal : ''}`,
        graded,
        possible: null,
  ...
  for (const u of unitKeys) {
    ...
    for (const ex of examsOf(cu)) addItem(u, ex.lesson, 'exam', { expected: true });
  }

  return { units, lessons, items, unmapped, manifestSeen, addItem, ensureLesson };
}

// A unit or lesson toggle has three states, not two, and collapsing them to two
// is how a teacher ends up flipping a switch that appears to already be where
// they want it. 'all' and 'none' are settled; 'mixed' means the columns under it
// disagree, which is a real and common state once anything has been set at a
// narrower scope.
function rollupLocks(itemList, keyOf) {
  const out = new Map();
  for (const it of itemList) {
    const k = keyOf(it);
    const r = out.get(k) || { key: k, items: 0, locked: 0, enforceable: 0 };
    r.items += 1;
    if (it.locked) r.locked += 1;
    if (it.lock_enforceable) r.enforceable += 1;
    out.set(k, r);
  }
  return [...out.values()].map((r) => ({
    ...r,
    state: r.locked === 0 ? 'none' : (r.locked === r.items ? 'all' : 'mixed'),
  }));
}
...
function buildCanonicalGradebook(classKey, opts) {
  ...
  const students = roster.map((s, idx) => {
    ...
    for (const it of itemList) {
      ...
      //
      // A lesson that carries an ACTUAL SCORE is the one case where ungraded
      // must not mean invisible. On AP Cybersecurity the lesson page IS the
      // graded activity: its CFU blocks are scored in the browser and
      // apcs-grade-reporter posts the percent as activity_type 'lesson'. That
      // score is stored, and GET /api/teacher/classes/:code/progress has always
      // shown it, so blanking it here made two teacher views disagree about the
      // same work. To a teacher that reads as "grades are not saving", which is
      // exactly how it was reported.
      //
      // Only the percent is surfaced. No points, no pass flag, no contribution
      // to earned/graded, and no new integrity row: a lesson percent has no
      // denominator anyone can point at on a page, and pricing it out of a
      // provisional 100 is the precise mistake SOURCE C below documents. Whether
      // these should carry real points is a weighting decision, not a rendering
      // one, and it is not made here.
      if (!it.graded) {
        const lessonPct = (c && c.score != null) ? round1(c.score) : null;
        cells[it.item_key] = {
          status: lessonPct != null ? 'attempted' : 'done',
          earned: null, possible: null, pct: lessonPct, passed: null,
          tries: c ? (c.attempts || 0) : 0,
          source: a ? 'attempts' : (b ? 'score_events' : 'progress'),
          possible_source: lessonPct != null ? 'percent' : null,
          completed, locked,
        };
        continue;
      }
      ...
      if (a && a.items > 0) {
        ...
        // SOURCE B. Real points from the ledger, and THEY WIN.
        //
        // One question is one point. The page counted the questions, marked them
        // right or wrong, and reported both halves: 3 out of 8 is what the
        // student actually saw and actually earned. No table can improve on that.
        //
        // This used to let an authored denominator override the reported pair
        // and rescale `earned` to match. That is backwards, and it made every
        // authored row a chance to overwrite a correct score with a guess: a
        // real 3/8 against an authored 15 was rendered as 6/15, a score the
        // student never got on a paper that never existed.
        //
        // Authored values still matter, but only where nothing was reported:
        // they price a column for a page that has no reporter yet. The moment a
        // page reports its own pair, that pair is the truth and the table is
        // ignored for that cell.
        //
        // A student served 5 of a 6 question bank quiz is out of 5, because that
        // is what they were asked. Rescaling to 6 would invent a question they
        // never saw and mark them wrong on it.
        const ratio = b.earned / b.possible;
        cell = {
          status: 'attempted',
          earned: round2(b.earned),
          possible: round2(b.possible),
          pct: round1(ratio * 100),
          tries: b.tries || b.items,
          items_attempted: b.items,
          items_total: b.items,
          source: 'score_events',
          possible_source: 'observed',
          completed, locked,
        };
        ...
        // priced against it. Without one there is NO pair, and none is invented.
        //
        // An earlier pass priced these out of a provisional 100 so that every
        // graded cell would show a pair. That was wrong, and it is worth saying
        // exactly how, because the arithmetic looked harmless: showing 17 as
        // "17 / 100" changes no percentage. What it changes is WEIGHT. A column
        // priced at 100 outweighs a real 5 mark quiz twenty to one in the
        // totals, so a single unpriced column could dominate a class average,
        // and a row total of "71 / 127" had a denominator nobody could point at
        // on any page.
        //
        // A denominator is the points possible on that page. If nobody has
        // authored one, the honest answer is that the points are unknown, not
        // that they are 100. The cell keeps its percentage and stays visible;
        // it simply cannot join a points sum until its real total is authored.
        // It is counted in items_percent_only and listed in
        // integrity.missing_denominators, which is the work queue for fixing it.
        //
        // AND IT HAS TO BE A PERCENT. progress.score is written by
        // /api/student/progress and /api/student/quiz from a number the PAGE
        // computed, and until 2026-09-11 neither route checked its range, so
        // rows on disk carry values like 483 (a lab scored against a
        // denominator counting only the questions answered so far). Source B
        // above cannot produce one, because points are clamped into [0, max]
        // per event; this branch is the only way a non-percent reaches a
        // reader. Capped here so the contract cannot report one, and the raw
        // value is carried on the cell so capping stays visible.
        const auth = it.possible;
        const priced = auth != null;
        const rawPct = round1(c.score);
        const outOfRange = rawPct < 0 || rawPct > 100;
        const pct = Math.max(0, Math.min(100, rawPct));
        cell = {
          status: 'attempted',
          earned: priced ? pointsFromRatio(pct / 100, auth) : null,
          possible: priced ? auth : null,
          pct,
          ...(outOfRange ? { pct_out_of_range: rawPct } : {}),
          tries: c.attempts || 0,
          source: 'progress',
        ...
        // COMPLETED, AND THE SCORE NEVER ARRIVED.
        //
        // Reaching here means all three sources are empty of a number for an
        // item the course says IS graded, because the ungraded branch above
        // returned already. So this is not "no score expected". A score was
        // expected, the student did the work, and nothing was recorded.
        //
        // That is a different fact from every other empty cell and it has to
        // look different. It rendered as a tick, which reads to a teacher as
        // done and fine, and that is how AP Cyber Ex2, Lab and Quiz completions
        // read as healthy all week while their pages posted no earned/possible
        // pair at all. A tick on lost data is worse than a blank: blank invites
        // a question, a tick closes one.
        //
        // It cannot be recovered by anything here. The number was never sent,
        // so no read-time fix invents it. The only cure is the page reporting
        // and the student resubmitting, which is exactly what the legend says.
        //
        // Deliberately NOT folded into integrity.ok. That flag means the CONFIG
        // is sound (denominators authored, activities mapped) and is stable for
        // a class; this count moves every time a student finishes something.
        // One flag that means two things would tell a teacher neither.
        cell = {
          status: 'done',
          score_missing: true,
          earned: null, possible: null, pct: null, passed: null,
          tries: c ? (c.attempts || 0) : 0,
          source: a ? 'attempts' : (b ? 'score_events' : 'progress'),
          completed, locked,
        };
        itemsScoreMissing += 1;
        scoreMissingKeys.add(it.item_key);
      }
    ...
    return {
      ...
      overall: {
        ...
        pct: graded > 0 ? round1(gradePct(earned, graded)) : null,
        earned,
        graded,
        ...
        // Graded work this student COMPLETED whose score never arrived. Not a
        // grade component: it cannot be, there is no number. It is the count of
        // cells a teacher must not read as healthy.
        items_score_missing: itemsScoreMissing,
      },
  ...
  for (const it of itemList) {
    ...
    it.class_avg_pct = g > 0 ? round1(gradePct(e, g)) : null;
  ...
  //  Resolved at read time against the same ladder students hit, never stored.
  //  Flipping a unit re-gates every column under it on the next read, with no
  //  backfill, and a lesson added to the course tomorrow inherits its unit.
  const gateRows = gateRowsStmt.all(cls.id, course);
  const aliasGroups = specAliasGroups();
  const bankKeys = new Set(
    bankLocationsStmt.all(course).map((b) => `${b.unit}|${b.lesson}|${b.activity_type}`)
  );
  for (const l of serverDeliveredLocations(aliasGroups)) {
    if (l.course === course) bankKeys.add(`${l.unit}|${l.lesson}|${l.activity_type}`);
  }
  const aliasIndex = aliasIndexFor(aliasGroups, course);
  // The class row shape resolveGate expects. A solo class has no teacher to open
  // anything, so it must resolve as self-study rather than as a locked class.
  const gcls = cls.course === course ? { id: cls.id, quiz_lock_default: cls.quiz_lock_default } : null;
  for (const it of itemList) {
    const inUnit = gateRows.filter((r) => r.unit === it.unit);
    //  ONE ladder, walked over every name this activity answers to, which is the
    //  same call routes/labs.js makes. A column with one name resolves exactly as
    //  it always did: resolveAliasGate over a single name IS resolveScopedGate.
    const names = aliasIndex.get(`${it.unit}|${it.lesson_ref}|${it.native_activity}`)
      || [it.native_activity];
    const row = names.map((n) => pickGateRow(inUnit, it.lesson_ref, n)).find(Boolean) || null;
    const g = resolveAliasGate(inUnit, gcls, it.lesson_ref, names);
    it.locked = !g.open;
    // Which scope decided it, so a UI can show a lesson toggle as inherited
    // from its unit rather than as a setting of its own, and a teacher clicking
    // it knows which row they are about to write.
    it.lock_scope = g.scope;
    it.lock_reason = g.reason;
    it.lock_explicit = !!row;
    // False means a lock here is decoration: the questions ship in the page.
    it.lock_enforceable = bankKeys.has(`${it.unit}|${it.lesson_ref}|${it.native_activity}`);
  }
  ...
  const integrity = {
    ...
    // Items where at least one student completed graded work and no score
    // was ever recorded. This is the page-reporter work queue: each key is
    // an activity whose page completes without posting earned/possible.
    scores_missing: scoreMissingKeys.size,
    score_missing_items: [...scoreMissingKeys].sort(),
  ...
  return {
    ...
    lessons: lessonList,
    items: itemList,
    // Everything a lock UI needs without a second round trip: the class default
    // (an empty row list means "all open" under one default and "all locked"
    // under the other, so the list alone cannot be read), the explicit rows a
    // teacher has written, and the roll-ups the unit and lesson toggles sit on.
    gates: {
      quiz_lock_default: cls.quiz_lock_default ? 1 : 0,
      rows: gateRows
        .map((r) => ({ ...r, open: r.open ? 1 : 0, scope: rowScope(r) }))
        .sort((a, b) => (a.unit + a.lesson + a.activity_type < b.unit + b.lesson + b.activity_type ? -1 : 1)),
      units: rollupLocks(itemList, (it) => it.unit),
      lessons: rollupLocks(itemList, (it) => `${it.unit}|${it.lesson_ref}`),
      locked_items: itemList.filter((it) => it.locked).length,
      // Columns a teacher has locked where the lock does nothing, because the
      // questions ship inside the page body. Named rather than counted so the
      // migration queue is readable: each one is a quiz to move onto the server
      // render path before its padlock means anything.
      locked_but_unenforceable: itemList
        .filter((it) => it.locked && !it.lock_enforceable)
        .map((it) => it.item_key),
    },
    students,
    summary: {
      ...
      class_pct: classGraded > 0 ? round1(gradePct(classEarned, classGraded)) : null,
      earned: classEarned,
      graded: classGraded,
      possible: classPossible,
      items_total: itemsTotal,
