...
const { resolveGate, pickGateRow, rowScope } = require('./activity-gate');
...
const labLocations = () => {
  try { return require('./lab-spec').all().map((sp) => ({
    course: sp.course, unit: sp.unit, lesson: sp.lesson_id,
    activity_type: sp.item_type || 'terminal-lab',
  })); } catch (e) { return []; }   // a missing or unreadable config must not blank a gradebook
};

//  Analysis activities are served by routes/analysis.js from config/analysis and
//  are gated there, so a lock on one is enforced by the server exactly as a lab
//  lock is. Added 2026-09-07 with the first of them, the AP Cyber 1.1 phishing
//  lab, which until that day kept its specimens and its answer key in the page
//  body and could not be withheld at all.
//
//  EVERY alias, not just the spec's own item_type. The column a teacher clicks
//  is 'lab' while the spec may call itself something else, and reporting only
//  the spec's name is how a real lock got drawn as decorative on the labs.
const analysisLocations = () => {
  try {
    const mod = require('./analysis-spec');
    const out = [];
    for (const sp of mod.all()) {
      for (const act of mod.aliases(sp)) {
        out.push({ course: sp.course, unit: sp.unit, lesson: sp.lesson_id, activity_type: act });
      }
    }
    return out;
  } catch (e) { return []; }
...
function buildCanonicalGradebook(classKey, opts) {
  ...
  for (const l of labLocations().concat(analysisLocations())) {
  ...
  for (const it of itemList) {
    ...
    const row = pickGateRow(inUnit, it.lesson_ref, it.native_activity);
    const g = resolveGate(row, gcls, it.native_activity);
    it.locked = !g.open;
