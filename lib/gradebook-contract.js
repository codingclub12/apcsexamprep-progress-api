...
function buildCanonicalGradebook(classKey, opts) {
  ...
  const manifestPoints = new Map();   // `${lesson}|${native}` -> { possible, items }
  ...
  for (const m of manifestRows) {
    ...
    const k = `${m.lesson_id}|${m.item_type}`;
  ...
  for (const it of items.values()) {
    ...
    if (manifestPoints.has(mk)) {
      it.possible = round2(manifestPoints.get(mk).possible);
      it.possible_source = 'manifest';
      it.manifest_items = manifestPoints.get(mk).items;
  ...
  const students = roster.map((s, idx) => {
    ...
    for (const it of itemList) {
      ...
      if (a && a.items > 0) {
        ...
        cell = {
          status: 'attempted',
          earned: round2(auth != null ? ratio * auth : b.earned),
        ...
        // SOURCE C. A percent with no points behind it. It can be priced only
        // against an authored denominator; without one it is shown but cannot
        // honestly join a points sum, so it is counted as percent-only and
        // reported rather than quietly folded in or quietly dropped.
        const auth = it.possible;
        cell = {
          status: 'attempted',
          earned: auth != null ? round2((c.score / 100) * auth) : null,
          possible: auth != null ? auth : null,
          pct: round1(c.score),
          tries: c.attempts || 0,
          source: 'progress',
          possible_source: auth != null ? it.possible_source : null,
          completed, locked,
        };
      } else {
