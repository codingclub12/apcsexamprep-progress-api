...
//  'drive' watches the Google Drive folders a teacher actually downloads. It is
//  its own source rather than another linkcheck because the surface differs: a
//  linkcheck finding is a page on the storefront and routes to shopify, and a
//  bundle finding is a file in Drive. Filing one as the other sends whoever
//  picks it up to the wrong place.
const SOURCES = ['smoke', 'ci', 'health', 'linkcheck', 'drive'];
...
const BUCKET_BY_SOURCE = { smoke: 'now', ci: 'now', health: 'now', linkcheck: 'week', drive: 'week' };
const SURFACE_BY_SOURCE = { smoke: 'api', ci: 'api', health: 'api', linkcheck: 'shopify', drive: 'drive' };
