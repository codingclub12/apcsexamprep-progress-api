...
function moduleMetaXml(modules) {
  const mods = modules.map((m) => {
    const items = m.items.map((it) => `      <item identifier="${it.id}">
        ...
        <new_tab>${it.newTab ? 'true' : 'false'}</new_tab>
...
function manifestXml({ manifestId, title, modules, resources }) {
  const org = modules.map((m) => {
    const items = m.items.map((it) =>
      `        <item identifier="${it.id}_o" identifierref="${it.ref}">
          <title>${esc(it.title)}</title>
        </item>`).join('\n');
    return `      <item identifier="${m.id}_o">
        <title>${esc(m.title)}</title>
${items}
      </item>`;
  }).join('\n');
...
function buildCanvasCoursePackage({ course, courseConfig, scope = 'unit', filter = null }) {
  ...
  function addWebLink(key, title, url) {
    ...
    resources.push({ id: rid, type: 'imswl_xmlv1p3', href, files: [href] });
    return rid;
  }
  ...
  return {
    ...
    summary: {
      ...
      link_count: resources.filter((r) => r.type === 'imswl_xmlv1p3').length,
      files: files.length,
      bytes: buffer.length,
    },
  };
