...
//  Every `NAME=document.getElementById('ELEMENT').value` with WHERE it appears.
//
//  Position matters and getting it wrong is not theoretical. These pages declare
//  a fresh `var t` inside each `if(n===N){...}` block, so one page had `t` bound
//  to s1-technique, s2-technique and s4-techniques in turn. A map from name to
//  element keeps only the last, which attributed `t==='spear'` to a TEXTAREA at
//  the bottom of the page and reported two perfectly good keys as ungettable.
function varBindings(html) {
  const out = [];
  for (const m of html.matchAll(/(\w+)\s*=\s*document\.getElementById\('([^']+)'\)\.value/g)) {
    out.push({ name: m[1], element: m[2], at: m.index });
  }
  return out;
}

//  Kept for callers that want the simple view; last binding wins, so only use it
//  where a name is known to be bound once.
function varToSelect(html) {
  const map = {};
  for (const b of varBindings(html)) map[b.name] = b.element;
  return map;
}

//  A comparison reads whatever that name was most recently bound to ABOVE it.
function credited(html) {
  const bindings = varBindings(html);
  const out = [];
  for (const m of html.matchAll(/(\w+)\s*===?\s*'([^']+)'/g)) {
    let bound = null;
    for (const b of bindings) {
      if (b.name !== m[1]) continue;
      if (b.at > m.index) break;
      bound = b;
    }
    if (bound) out.push({ varName: m[1], select: bound.element, value: m[2] });
  }
  return out;
}
...
//
//  Matched by SHAPE, not by helper name. The first version looked for tMatch and
//  tCount, which are what two of these pages call the helper. The Topic 1.4 lab
//  calls it `tc`, so the protection silently did not apply there at all: the
//  gate reported zero keyword lists, which reads exactly like a page that has
//  none. The shape (an identifier called with a variable and an array of string
//  literals) is the same everywhere and does not depend on what anyone named it.
const KEYWORD_CALL = /\b(\w+)\(\s*(\w+)\s*,\s*\[((?:\s*'[^']*'\s*,?)+)\]/g;

function keywordLists(html) {
  const out = [];
  for (const m of html.matchAll(KEYWORD_CALL)) {
    const keys = [...m[3].matchAll(/'([^']*)'/g)].map((k) => k[1]);
    if (keys.length < 2) continue;          // a one-element array is not a rubric
    out.push({ helper: m[1], varName: m[2], keys });
  }
  return out;
}
...
module.exports = {
  ...
  flat, selects, varBindings, varToSelect, credited, keywordLists, check,
};
