...
function priceRow(r, cal, rpmUsd, days) {
...
//  SCENARIOS: "a 28 student class is worth $X a year". This is the shape the
//  pricing conversation actually needs, and it is deliberately derived from a
//  PER STUDENT rate rather than from any one class, so that no single school's
//  behaviour sets the price for every school.
//
//  The per student rate is pooled across non-excluded, non-suppressed classes
//  that produced a priced figure, weighted by active students.
function scenarios(priced, rpmUsd, sizes) {
  const wanted = Array.isArray(sizes) && sizes.length ? sizes : [25, 28, 50, 55, 100, 110];
  let pvSum = 0, studentSum = 0, classes = 0;
  let anyMeasured = false;
  for (const r of priced) {
    if (r.tier === 'excluded' || r.suppressed) continue;
    if (r.est_annual_pageviews == null || r.active_students <= 0) continue;
    pvSum += r.est_annual_pageviews;
    studentSum += r.active_students;
    classes += 1;
    if (r.pageview_basis === 'measured') anyMeasured = true;
  }
  if (!classes || studentSum <= 0) {
    return {
      basis: 'none',
      reason: 'no class produced a priced pageview figure in this window',
  ...
  const pvPerStudent = pvSum / studentSum;
  const band = anyMeasured ? BAND.measured : BAND.estimated;
  return {
    basis: anyMeasured ? 'measured_and_estimated' : 'estimated_only',
    from_classes: classes,
    from_active_students: studentSum,
    annual_pageviews_per_active_student: Math.round(pvPerStudent),
    rows: wanted.map((n) => {
      //  Same rule as a class row: the reported pageview figure is what gets
      //  priced, so the table ties out to itself.
      const pv = Math.round(pvPerStudent * n);
      if (rpmUsd == null) {
        return {
          students: n,
...
function report(opts) {
  ...
  const priced = f.rows.map((r) => priceRow(r, cal, rev.rpm_usd, days));
  ...
  return {
    ...
    scenarios: scenarios(priced, rev.rpm_usd, o.sizes),
