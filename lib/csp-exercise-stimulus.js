...
function renderStimulus(src, id) {
  ...
  // Only a time column is kept on one line. Other logs put a name there
  // ("AttendanceCast (skip-risk predictor)"), which has to be free to wrap.
  const whenCol = log.columns[1] === 'Moment';
  const body = log.rows.map((r) => '          <tr>' + r.map((c, j) => {
    const cls = j === 0 ? ' class="row-id"' : (j === 1 && whenCol) ? ' class="row-when"' : '';
