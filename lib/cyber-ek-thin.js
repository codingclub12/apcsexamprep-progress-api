...
const PROSE = [
  // FIRST, because order decides this one. "CED 1.5.B.3 covers both alerting and
  // corrective action" is a code with the word CED already in front of it. The
  // subject-position rule further down turns the bare code into "the CED" and
  // leaves the original word standing, producing "CED the CED covers both
  // alerting and corrective action". Consuming the pair as one unit has to
  // happen before that rule gets a look. Four of the five Unit 1 lesson pages
  // carry this shape, so it is not a one-page typo, and it was found by reading
  // the changed sentences rather than by any check.
  [/\b(?:The |the )?CED (?:EK )?\d\.\d\.[A-C](?:\.\d)?\b/g, 'the CED'],
...
function thin(body) {
  ...
  //
  //     This runs INSIDE outsideProtected. It used to run globally, which made
  //     it the one step that could reach into the EK coverage table and delete
  //     rows from it, protection or no protection: the second rule deletes any
  //     row-final <td> that opens with a code. It never fired there on 1.1 or
  //     1.4, because those coverage tables put the code in a first cell
  //     carrying class="term", so a bare <td> never matched. That is luck of
  //     markup, not a guarantee, and the guarantee is what the protection
  //     mechanism is for.
  b = outsideProtected(b, (chunk) => chunk
  ...
  //
  //     Deliberately NOT wrapped in outsideProtected, unlike step 2. These are
  //     literal paired strings, and protecting one half of a pair while
  //     rewriting the other is the exact failure the pairing exists to prevent.
  //     Global is the safe direction here; for step 2 it was the unsafe one.
  b = b
