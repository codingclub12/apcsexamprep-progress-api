...
//  ── MOJIBAKE ────────────────────────────────────────────────────────────────
//  This used to be a detector of its own: a lead set, a cp1252 table, a byte
//  reverser and a width loop, all living here. It was the BEST of the three the
//  repo had, because it was the only one that knew live rendered HTML carries
//  the cp1252 form, and it said so in a comment aimed at
//  smoke/encoding-guard.js. That comment was right, it was committed on
//  2026-08-26, and the other two guards were still wrong on 2026-09-03. Eight
//  days, in the same repository, with the answer written down. That is the
//  argument for one module rather than a fourth opinion.
//
//  It still had the hole all three shared: widths 3 and 2 with a lead set that
//  stopped at U+00E2, so a corrupted EMOJI was invisible to it at every width. A
//  4-byte character corrupts into four. That is now covered, because the lead
//  range and the run width are derived from UTF-8 in lib/mojibake.js instead of
//  being listed from samples.
const mojibake = require('./mojibake.js');

function detectMojibake(text, cap = 5) {
  //  Shape kept as it was: { raw, fixed, index }, because checkPage below and
  //  smoke/site-crawl.js both read `raw`.
  return mojibake.detect(text, cap)
    .map((h) => ({ raw: h.run, fixed: h.fixed, index: h.index }));
}
