...
function renderJsonLd(entry, kind) {
  ...
  if (entry.causes && entry.causes.length) {
    qa.push({
      // Strips whichever title prefix this page uses. Five error pages were
      // retitled "Java error in Greenfoot: X" to stop them competing with the
      // AP CSA java-errors cluster, and a prefix pattern that only knew the old
      // shape produced "What usually causes Java error in Greenfoot: cannot
      // find symbol?" in structured data on all five.
      q: `What usually causes ${entry.title.replace(/^Java error(?: in Greenfoot)?: /, '')}?`,
