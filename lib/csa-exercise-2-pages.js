...
function renderExercise2(bank) {
  ...
  const items = bank.questions.map((q, i) => {
    ...
    const fb = order.map((src, j) => {
      ...
      const whyText = q.why[LETTERS[src]];
      // Every unit2/3/4 seed already opens its correct-answer why-text with
      // the literal word "Correct." (checked: 226 of 228 questions), so
      // adding the label again here rendered "Correct. Correct. ..." live.
      // Skip the label only when the seed text already carries that exact
      // lead-in, so the 2 questions that do not (and any future ones) still
      // get it from the renderer. The period is required in the match: some
      // distractor why-text legitimately starts with the word "Correct" as
      // English prose ("Correct results, but it scans the whole array..."),
      // and matching that loosely would silently drop its "Incorrect." label.
      const alreadyLabeled = /^(Correct|Incorrect)\.\s/.test(whyText);
      const label = right ? 'Correct.' : 'Incorrect.';
      return `        <div id="${qid}-fb-${L}" class="mcq-feedback ${right ? 'correct-fb' : 'incorrect-fb'}">`
        + `${alreadyLabeled ? '' : label + ' '}${esc(whyText)}</div>`;
    }).join('\n');
