...
//  THE NAMES A GATE ROW MAY USE FOR THIS LAB.
//
//  A spec declares item_type 'terminal-lab'. The gradebook column a teacher
//  clicks is 'lab', because utils.js lists 'lab' in every cyber unit's
//  activities. The two names exist for a denominator collision recorded in
//  lib/gradebook-contract.js, not because a teacher thinks of them as two
//  things, so anything asking "is this lab open" has to ask about both.
//
//  This lived in routes/labs.js as a private constant until a second caller
//  needed it. The second caller is the gradebook, which drew a padlock from one
//  name while the student hit a ladder built from two, and the two disagreed in
//  the only direction that matters: the board said open and the student was
//  refused. Same shape as analysis-spec.aliases, deliberately, so a third
//  activity with two names has an obvious place to go.
function aliases(spec) {
  const own = (spec && spec.item_type) || 'terminal-lab';
  return own === 'lab' ? ['lab'] : [own, 'lab'];
}

module.exports = { get, all, graded, errors, summary, forBrowser, validate, load, aliases, LAB_DIR };
