...
//  AFTER an import, byte equality is the WRONG assertion and this is the right
//  one. Shopify reformats HTML on save: measured 2026-09-06, the two bundle
//  pages came back 2 and 3 bytes longer than the sheets sent them, and the
//  entire delta was newlines ("<li><strong>" became "<li>\n<strong>"). Every
//  non-whitespace character was identical and in order.
//
//  So this tolerates whitespace and nothing else. It still catches the failure
//  the whole guard exists for, a list item silently deleted three screens below
//  the edit, because a deletion changes the non-whitespace character count and
//  breaks the recovery check. Whitespace between tags is the only difference
//  that does not change what a reader sees.
function verifyAfterImport(before, block, body) {
  const problems = [];
  const strip = (x) => x.replace(/\s+/g, '');
  const b = strip(before), k = strip(block), y = strip(body);

  if (y.length !== b.length + k.length) {
    problems.push(`${y.length} non-whitespace characters, expected ${b.length + k.length} (${b.length} before plus ${k.length} block). Content was added or lost, not just reformatted.`);
    return problems;
  }
  if (!y.includes(k)) {
    problems.push('the authored section is not present intact once whitespace is set aside');
    return problems;
  }
  if (y.replace(k, () => '') !== b) {
    problems.push('removing the block does not return the pre-import body. Something in the original was changed or deleted.');
  }
  return problems;
}


//  REPLACE a span rather than insert before an anchor. Same proof obligation and
//  the same reason for it: the caller hands over the live body and the exact
//  bytes it expects to remove, and gets back a body that differs in exactly that
//  one place or a refusal.
//
//  Insertion could rely on a length check because it only ever added. A
//  replacement can SHORTEN the body, which is the shape the /pages/join incident
//  actually had, so the round trip is the load-bearing half here: swap the new
//  span back for the old one and the original must return byte for byte.
function replaceSpan({ live, removed, added }) {
  const problems = [];
  const hits = live.split(removed).length - 1;
  if (hits !== 1) {
    problems.push(`the span to remove occurs ${hits} times, expected exactly 1. Re-aim this rather than guess.`);
    return { problems };
  }
  if (live.includes(added)) {
    problems.push('the replacement is ALREADY in the live body: nothing to do, and running again would duplicate it');
    return { problems };
  }

  const out = live.replace(removed, () => added);
  problems.push(...verifyReplacement(live, removed, added, out));
  return { problems, out };
}

//  Factored out for the same reason verifyInsertion was: replaceSpan builds its
//  own `out`, so a test that can only call replaceSpan can never hand it a
//  corrupted body and prove these two fire. A mutation that moves a byte without
//  changing the length is exactly what the round trip is for, and it is
//  invisible to a suite that cannot construct one.
function verifyReplacement(live, removed, added, out) {
  const problems = [];
  if (out.length !== live.length - removed.length + added.length) {
    problems.push(`length is ${out.length}, expected ${live.length - removed.length + added.length}. Something other than the intended swap happened.`);
  }
  if (out.replace(added, () => removed) !== live) {
    problems.push('swapping the replacement back for the original does not return the live body byte for byte');
  }
  return problems;
}

module.exports = { splice, verifyInsertion, verifyAfterImport, checkAuthored, replaceSpan, verifyReplacement };
