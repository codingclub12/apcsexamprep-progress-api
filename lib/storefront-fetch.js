...
//  ---------------------------------------------------------------------------
//  503 JOINED 429 ON 2026-09-18, on the same reasoning and a second measurement.
//  verify-csa-frq-archive-live walks 142 storefront requests in one pass. Run
//  three times it failed on 7, then 1, then a different 1, always with 503 and
//  never the same handle twice; every one of those handles answered 200 with the
//  expected body when fetched on its own a moment later. So Shopify sheds load
//  with 503 as well as 429, and a verifier that goes red at random is a verifier
//  that gets ignored, which is how a guard gets switched off.
//
...
//
//  That rule held against this change, and it is worth saying so. The work
//  below was going to widen this set to the whole transient family, 408, 500,
//  502 and 504, on the argument that every read here is an idempotent GET. The
//  argument is fine and the measurement was missing: no run in this repo has
//  produced any of the four. A 500 in particular can be a durable fact about a
//  page, and retrying it four times with a growing wait makes a genuinely
//  broken page nine seconds slower to report. So the set is unchanged and the
//  dropped connection below is what actually got fixed.
...
//  THE THROWN CASE IS RETRIED TOO, and it is the half no retry work has
//  covered yet. PR 752 and PR 786 both widened what happens to a transient
//  STATUS CODE; neither touched this. rawOnce() throws NotThePage when curl
//  itself fails, on a connection reset or a timeout or a DNS hiccup, and raw()
//  never caught it. So the one failure mode with no HTTP code at all got ZERO
//  retries while a 429 got four.
//  That asymmetry made verify-quiz-mount-unpin-live.js read 22 of 23 on
//  2026-09-21 while three consecutive re-runs read 23 of 23: one page out of
//  twenty-three lost its connection, and the count looked like a regression.
//  The contract does not change. After the last attempt the original error is
//  re-thrown, so every caller that relies on the throw still gets it; only a
//  blip a second try would have survived is now survived.
function raw(target, opts) {
  const o = opts || {};
  const asked = o.retryAttempts == null ? RETRY_ATTEMPTS : o.retryAttempts;
  //  At least one request, whatever was asked for. The old shape called
  //  rawOnce() once before the loop, so retryAttempts: 0 still fetched; this
  //  shape would run the loop zero times and return null, and a caller writing
  //  0 to mean "no retries" would get a TypeError on r.body rather than an
  //  answer. NaN floors here too, since NaN >= 1 is false.
  const attempts = asked >= 1 ? asked : 1;
  let r = null;
  let thrown = null;
  for (let i = 0; i < attempts; i += 1) {
    if (i) sleepSync(RETRY_WAIT_MS * i);
    try {
      r = rawOnce(target, o);
      thrown = null;
    } catch (e) {
    ...
    if (!RETRY_CODES.has(r.code)) return r;
  }
...
function rawOnce(target, opts) {
  ...
  //  TAB DELIMITED, and that is a correction rather than a style choice. The
  //  fields used to be space separated and split on /\s+/ after a trim, which
  //  works only while every field is non-empty. %{redirect_url} is EMPTY on the
  //  common case of no redirect, and a trailing empty field does not survive
  //  trim, so the caller would silently read one field short. A tab cannot
  //  appear in a URL or a status code, so the field count is fixed.
  const args = ['-sS', '--max-time', String(o.timeout || 45), '--compressed',
    '-o', tmp, '-w', '%{http_code}\t%{url_effective}\t%{redirect_url}'];
  if (o.follow !== false) args.push('-L');
  //  HEAD is for a caller that wants a status without paying for 350KB. curl
  //  needs --head for it: -X HEAD alone leaves curl waiting for a body that
  //  never arrives, until the timeout. The file then holds the RESPONSE
  //  HEADERS rather than the page, which is why looksReal() must not be asked
  //  about a HEAD body and page() refuses the method outright below.
  if (o.method === 'HEAD') args.push('--head');
  else if (o.method && o.method !== 'GET') args.push('-X', o.method);
  args.push(url);
  //  execFileSync, not a shell: a command string let the shell eat the -w format.
  let status;
  try {
    status = cp.execFileSync('curl', args, { encoding: 'utf8' }).split('\t');
  } catch (e) {
  ...
  const field = (i) => String(status[i] == null ? '' : status[i]).trim();
  return {
    url,
    code: field(0),
    finalUrl: field(1) || url,
    //  Where this response points NEXT, empty when it is not a redirect. A
    //  caller that follows hops itself, to count the chain, needs this and
    //  would otherwise have to read response headers. link-graph and
    //  site-crawl both do exactly that, and the chain LENGTH is the thing they
    //  exist to measure, so losing it is not an acceptable cost of moving them
    //  onto this module.
    redirectUrl: field(2),
    body,
  };
...
//  ── STATUS, FOR THE THINGS THAT HAVE NO BODY TO JUDGE ───────────────────────
//  page() protects a body fetch: it refuses anything that is not provably a
//  rendered page, so a bot challenge cannot pass as content. A REDIRECT has no
//  body, so none of that machinery applies, and the gap had a cost on
//  2026-09-06: a check written with node https.get reported the old handle of a
//  renamed page as 403 while curl said 301, three times running. The 403 was the
//  challenge. Believed, it would have read as "the redirect is not there" and
//  sent somebody hunting a bug that did not exist.
//
//  So this exists to stop the next person hand-rolling one. It is curl, with no
//  User-Agent, exactly like every other fetch here, and it does NOT follow the
//  redirect: following it is what turns a 301 into the 200 of somewhere else and
//  loses the fact you were checking for.
//
//  It reports `challenged` rather than throwing. A 403 can be a real answer, and
//  a helper that threw on one would be unusable for asserting that a gated page
//  is still gated. The caller decides; this only makes the distinction visible.
function status(target, opts) {
  const o = opts || {};
  const url = /^https?:\/\//.test(target) ? target : STORE + target;
  const tmp = path.join(os.tmpdir(),
    'apcse-st-' + process.pid + '-' + (SEQ++) + '-' + Date.now() + '.html');
  const args = ['-sS', '--max-time', String(o.timeout || 30), '--compressed',
    '-o', tmp, '-w', '%{http_code} %{redirect_url}', url];
  let out;
  try {
    out = cp.execFileSync('curl', args, { encoding: 'utf8' }).trim().split(/\s+/);
  } catch (e) {
    try { fs.unlinkSync(tmp); } catch (e2) {}
    throw new NotThePage(url + ': curl failed (' + String(e.message).split('\n')[0] + ')',
      { code: null, url });
  ...
  return {
    url,
    code: out[0],
    location: out[1] && out[1] !== 'null' ? out[1] : null,
    challenged: CHALLENGE.test(body),
  };
}
...
function page(target, opts) {
  //  A HEAD body is the response headers, not the page, so every marker test
  //  below would refuse it and the caller would read that as "the storefront is
  //  down". Refuse the METHOD instead, which says what is actually wrong. A
  //  caller wanting a status without a body uses raw({ method: 'HEAD' }).
  if (opts && opts.method === 'HEAD') {
    throw new NotThePage('page() cannot verify a HEAD response, because its body is '
      + 'the response headers rather than the page. Use raw({ method: "HEAD" }) '
      + 'when a status is all you need.', { code: null, url: target });
  }
...
//  ---- pageBody() CAN BE STALE, AND updated_at CANNOT TELL YOU SO ----------
//  /pages/<handle>.json is served through Shopify's own page cache. The etag
//  says so out loud: page_cache:<id>:PageDetailsController:<hash>. A query
//  string does not bust it, because the cache keys on the path.
//
//  Measured 2026-09-06. A Matrixify import landed at 21:51:33Z and this endpoint
//  went on serving the PRE-import body, with the PRE-import updated_at, for
//  about a minute afterwards. That second detail is the trap: updated_at is the
//  obvious way to ask "did my write land", and it arrives inside the same stale
//  response, so it confirms whatever the cache is holding. A verifier read it,
//  saw the old body and the old timestamp agreeing with each other, and reported
//  that a successful import had not run. The operator re-imported on that advice.
//
//  So: NEVER conclude "my write did not land" from this endpoint alone. Prove
//  freshness against something with a different cache. The rendered page
//  (page() above) is the one to hand, and settled() below is the cheap version:
//  poll until the value stops moving.
//
//  Note the two read paths have opposite defects, which is why neither alone is
//  enough. The rendered page is FRESH but Cloudflare rewrites addresses in it
//  (see cloudflareRewritten above). The json is CLEAN but can be a minute
//  behind. Use the json for bytes you will write back, and the rendered page to
//  prove those bytes are current.
//
//  ---- decodeCfEmails ------------------------------------------------------
//  Reading an address out of a rendered page needs the Cloudflare cipher undone:
//  data-cfemail is hex, first byte is the XOR key, rest is the address. Without
//  this a live check on a page carrying an address can only ever say "absent",
//  which is the false negative that shape of check is most likely to produce.
...
//  Read pageBody() until it stops changing, so a cache mid-flight cannot be
//  mistaken for a settled answer. Returns { page, reads, settled }. settled is
//  false when it ran out of attempts while still moving, and a caller must treat
//  that as "unknown", never as "unchanged".
function pageBodySettled(handle, opts) {
  opts = opts || {};
  const attempts = opts.attempts || 6;
  const waitMs = opts.waitMs || 10000;
  let prev = null, page = null, reads = 0, stable = 0;
  for (let i = 0; i < attempts; i++) {
    page = pageBody(handle);
    reads++;
    const sig = page.updated_at + ':' + page.body_html.length;
    if (prev !== null && sig === prev) {
      stable++;
      if (stable >= 1) return { page, reads, settled: true };
    } else { stable = 0; }
    prev = sig;
    if (i < attempts - 1) sleepSync(waitMs);
  }
  return { page, reads, settled: false };
}
...
module.exports = { STORE, MARKERS, CHALLENGE, NotThePage, raw, rawOnce, page, status, looksReal, refusal,
  RETRY_CODES, RETRY_ATTEMPTS,
  CF_REWRITE, cloudflareRewritten, pageBody, pageBodySettled, decodeCfEmails };
