...
//  ── WHY THERE ARE TWO DEPTH NUMBERS ────────────────────────────────────────
//  Measuring content clicks from the homepage ALONE gives a number nobody
//  should act on, and the reason is a real property of this site rather than a
//  bug. The homepage links its course hubs in its own body, but those same URLs
//  sit in the mega-menu on all 2,063 pages, so ubiquity correctly calls them
//  chrome. Traversal then refuses to walk through the site's main hubs and
//  reports almost everything unreachable, which is true of the metric and false
//  of the site.
//
//  So depth is measured from the NAV FRONTIER: the homepage plus every target
//  the navigation reaches, all of which are one click away from anywhere. The
//  question that answers is the one worth asking, which is how far the content
//  graph extends once a reader is standing somewhere the nav can put them.
//
//  The homepage-only figure is still computed, because the gap between the two
//  is exactly the site's dependence on its own navigation.
function depths(graph, roots = ['/']) {
  const d = new Map();
  const seeds = (Array.isArray(roots) ? roots : [roots]).filter((p) => graph.nodes.has(p));
  if (!seeds.length) return d;
  for (const s of seeds) d.set(s, 0);
...
function analyze(graph) {
  //  Primary metric: content clicks from anywhere the nav can drop a reader.
  const navFrontier = ['/', ...graph.chromeTargets];
  const depth = depths(graph, navFrontier);
  //  Kept for contrast: content clicks from the homepage and nothing else.
  const homeDepth = depths(graph, ['/']);
  ...
  //  429 and 503 are the crawler being throttled, not the page being broken.
  //  Reporting them as dead pages sends somebody to check a URL that serves 200
  //  on the next request, which is how a report teaches people to skim it.
  const THROTTLED = new Set([429, 503]);
  const broken = all.filter((x) => x.crawled && x.status >= 400 && !THROTTLED.has(x.status));
  const throttled = all.filter((x) => x.crawled && THROTTLED.has(x.status));
  ...
  return {
    ...
    depth, homeDepth, navFrontierSize: navFrontier.length,
    orphans, nearOrphans, deadEnds, unreachable, broken, throttled, dangling,
...
function resolveClusters(live) {
  ...
  const UNIT_WORDS = ['unit', 'bi', 'big-idea', 'lesson', 'topic'];
  ...
  function stemVariants(family) {
    ...
    const m = family.match(/^(.*?)-((?:unit|big-idea|bi|lesson|topic))-?(\d+(?:-\d+)?)$/);
    if (!m) return [];
  ...
  for (const c of clusters.values()) {
    ...
    //  ── WHEN THE HUB IS ONE OF THE MEMBERS ──────────────────────────────────
    //  For a lesson family the hub IS a member: ap-csa-lesson-2-3 is headed by
    //  ap-csa-lesson-2-3-expressions-assignment, with the exercises beneath it.
    //  For a cyber unit family it is NOT: every member is an activity and the
    //  hub lives under the other prefix.
    //
    //  What separates them is not membership, it is whether the candidate is an
    //  ACTIVITY. A quiz, lab, exam or exercise is never the hub of anything, so
    //  a member that carries none of those suffixes and has the shortest handle
    //  is the bare topic page and heads the family. Blanket-excluding members
    //  reported 53 CSA lesson families as hubless; blanket-including them let
    //  ap-cyber-unit-3-exam head its own unit.
    //  Scoped to NUMBERED families only. The 'stem plus descriptive slug'
    //  convention exists for units, lessons and topics and nowhere else, so
    //  applying it to an unnumbered family lets any member with a short handle
    //  crown itself: ap-csp-written-response-guide would head its family and
    //  the 'guide is not a hub' rule would stop meaning anything.
    const ACTIVITY = /-(exercise(-\d+)?|quiz|lab|terminal-lab|exam|case-file(-\d+)?|project|scenario-practice|practice)$/;
    const stemsEarly = stemVariants(c.family);
    if (stemsEarly.length) {
      const memberHub = c.members
        .filter((m) => m.handle && !ACTIVITY.test(m.handle)
          && stemsEarly.some((st) => m.handle === st || m.handle.startsWith(st + '-')))
        .sort((x, y) => x.handle.length - y.handle.length)[0];
      if (memberHub) { c.hub = memberHub; c.hubAlias = true; continue; }
    }
