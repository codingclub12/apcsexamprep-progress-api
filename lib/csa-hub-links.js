...
function section(p, lessons, builtCount) {
  return `<div class="${p}-exercises" style="margin-bottom:40px!important;">`
    + `<h2 class="${p}-section-title">Coding Practice</h2>`
    ...
    + 'Write real Java in the browser and submit it. Everything here is graded against hidden '
    + 'test cases, and your score reports to your teacher automatically. '
    + 'Exercise 1 is write-it-from-scratch, Debug hands you somebody else\'s broken attempt to fix, '
    + 'and FRQ is the shape the exam actually asks for. '
    + `${builtCount} of ${lessons.length} lessons are open so far; the rest unlock as they are built.</p>`
...
// ── RE-RUNNING AGAINST AN ALREADY PATCHED HUB ────────────────────────────────
// The first version of this module refused any body that already carried an
// exercises section, on the grounds that inserting a second one is worse than
// doing nothing. That was right, and it also made the section unmaintainable:
// the four hubs are patched and live, and every new practice type (Debug and
// FRQ on 2026-08-25) has to reach a body that already has the old section in it.
//
// So the refusal becomes a REPLACEMENT, and the replacement is deliberately
// narrow. It removes exactly one machine-generated section and only if it can
// prove that is what it found:
//
//   1. the opening tag is the exact one section() writes,
//   2. its matching close is located by counting nested <div> and </div> from
//      that point rather than by matching the next </div>, which would cut the
//      block in half and leave the rest of the page inside it,
//   3. the removed span must contain the generated heading, so a hand-written
//      block that happens to use the class name is never silently deleted.
//
// Anything else throws. A hub body is a live page, and the failure mode of
// guessing here is deleting somebody's content.
function stripSection(body, p) {
  const open = `<div class="${p}-exercises" style="margin-bottom:40px!important;">`;
  ...
  let i = at;
  let end = -1;
  const OPEN_RE = /<div[\s>]/g;
  const CLOSE = '</div>';
  while (i < body.length) {
    OPEN_RE.lastIndex = i;
    const nextOpen = OPEN_RE.exec(body);
  ...
  if (end === -1) {
    throw new Error(`the existing ${p}-exercises section has no matching close tag; refusing to cut a live body`);
  }

  const span = body.slice(at, end);
  if (span.indexOf(`<h2 class="${p}-section-title">`) === -1) {
    throw new Error(`the existing ${p}-exercises block does not look machine generated `
      + '(no generated section heading); refusing to delete it');
  }

  const out = body.slice(0, at) + body.slice(end);
  if (out.indexOf(`class="${p}-exercises"`) !== -1) {
    throw new Error(`more than one ${p}-exercises section is present; resolve that by hand`);
  }
  return { body: out, removed: true, bytes: span.length };
}
...
function fixCta(body, p) {
  ...
  //
  // BOTH SPELLINGS ARE THE SAME DEFECT. Measured 2026-08-24 against a Matrixify
  // Pages export of all four hubs, cross-checked against the credential-free
  // rendered dump, which came back BYTE-IDENTICAL: the stored body of Units 1
  // and 2 really does contain `class="u1-cta"&gt;`, with the `>` escaped. An
  // earlier version of this module read that escaping as proof of a rendered
  // scrape and refused the only input that exists.
  ...
  let out = body.replace(broken, `$1<div class="${p}-cta">`);

  // THE OTHER HALF OF THE SAME BREAKAGE.
  // The stored body counts balanced only because two errors cancel: the CTA
  // block's `</div>` has no opener, so it closes `#uN-hub` early, and the
  // wrapper's own closing tag is gone entirely. That is why the units strip and
  // the unit nav render OUTSIDE the scoped wrapper on the live page today.
  // Restoring the opener alone leaves the body one div short, so the wrapper
  // close is restored too: after the nav, before the JSON-LD that trails it.
  const opens = (out.match(/<div[\s>]/g) || []).length;
  const closes = (out.match(/<\/div>/g) || []).length;
  if (opens === closes + 1) {
    const navEnd = out.lastIndexOf('</nav>');
    if (navEnd === -1) {
      throw new Error('the CTA repair leaves the body one div short and there is no </nav> to close the '
        + 'wrapper after; refusing to place a closing tag by guess');
    }
    const at = navEnd + '</nav>'.length;
    out = `${out.slice(0, at)}\n</div>${out.slice(at)}`;
  }
  const bal = (out.match(/<div[\s>]/g) || []).length - (out.match(/<\/div>/g) || []).length;
  if (bal !== 0) throw new Error(`the CTA repair left the body ${bal} div(s) out of balance`);
  return { body: out, fixed: 1 };
}
...
function check(inBody, outBody, p, lessons, ctaFixed, relinked) {
  ...
  // A repaired body must balance exactly; an untouched one must not drift.
  const expected = ctaFixed ? 0 : opens(inBody) - closes(inBody);
...
function build(inBody, liveHandles) {
  ...
  // An already patched hub is replaced rather than refused. See stripSection.
  const stripped = stripSection(inBody, p);
  const replacedSection = stripped.removed;
  const baseBody = stripped.body;

  const parsed = parseLessons(baseBody, p);
  if (!parsed.length) {
  ...
  const lessons = parsedFixed.map((l) => ({
    ...l,
    // Four practice types per lesson, in the order a student meets them:
    // write it, write it again applied, fix someone else's, then sit the exam
    // shape. Debug and FRQ were added 2026-08-25, when their 53 pages each
    // became importable; before that every one of those chips would have
    // rendered locked, which is exactly what this list does for anything not in
    // the live handle set.
    exercises: [
      ...
      { label: 'Debug', handle: `${l.handle}-debug`, built: liveHandles.has(`${l.handle}-debug`) },
      { label: 'FRQ', handle: `${l.handle}-frq`, built: liveHandles.has(`${l.handle}-frq`) },
    ],
  }));
  ...
  return {
    ...
    relinked,
    ctaFixed: cta.fixed,
    // True when an older generated section was removed and rebuilt. The
    // operator needs to know the difference between "this hub gained a section"
    // and "this hub's section was replaced", because only the second one can
    // lose links that were live a moment ago.
    replacedSection,
...
module.exports = {
  ...
  fixCta, check, requiredMarkers, anchors, stripSection,
};
