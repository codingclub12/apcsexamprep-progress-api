...
// Points are whole numbers when the denominator is. Imported rather than
// restated so this view and the contract cannot round differently, which is
// exactly how the two drifted apart before.
// gradePct is the grade rule itself: points over points, null when nothing was
// attempted. Imported for the same reason, one level up. The two builders had
// already drifted on the DIVISION, not just the rounding: this one averaged
// percentages where the contract summed points.
const { pointsFromRatio, gradePct } = require('./gradebook-contract');
...
// student (retry_override beats the class policy): best ratio when the best
// attempt counts for THIS item type, first attempt when it does not.
//
// The decision comes from retry-policy.js rather than being spelled out here,
// so this pass and the row-by-row JS paths cannot drift. It is activity-type
// aware: under mode 'practice' a cfu is redoable while a quiz in the same class
// is one shot, which a single boolean could not express.
...
function buildGradebook(classKey, opts) {
  ...
  const unitOrder = [];
  // The key is UNIT scoped. Cyber gives every unit a lesson called 'exam' and one
  // called 'case-file', so a `lesson|activity` key collapsed all five units'
  // exams into a single column and all five case files into another: eight
  // columns vanished from the grid, and one unit's exam cell overwrote another's.
  // This is the same collision course_unit_denominators exists for.
  //
  // A row observed with no unit falls back to the unit the lesson implies, so an
  // out-of-config lesson still lands somewhere stable rather than under ''.
  const ensureItem = (lesson, activity, unit, expected) => {
    const u = unit || 'unit-' + String(lesson).split('.')[0];
    const key = `${u}|${lesson}|${activity}`;
    if (!items.has(key)) {
      items.set(key, {
        key, lesson_id: lesson, activity, unit: u,
  ...
  if (cfg && cfg.units) {
    for (const unitId of Object.keys(cfg.units)) {
      ...
      for (const ex of examsOf(u)) ensureItem(ex.lesson, 'exam', unitId, true).label = ex.label || 'Unit Exam';
    }
  }
  ...
  `).all(cls.id, course);
  // Keyed by UNIT as well, for the same reason the item key is: without it a
  // student's unit-1 exam row and their unit-3 exam row shared one entry and the
  // last one written silently replaced the rest.
  const progCell = new Map(); // `${sid}|${unit}|${lesson}|${activity}` -> row
  for (const p of progRows) {
    const it = ensureItem(p.lesson, p.activity_type, p.unit, false);
    if (p.score != null || p.completed) it.observed = true;
    progCell.set(`${p.student_id}|${it.unit}|${p.lesson}|${p.activity_type}`, p);
  }
  ...
  //  `possible` is the whole course: the sum over every graded column that has
  //  an authoritative denominator. It is a property of the CLASS, so every
  //  student reports the identical number, and it is what answers "how much of
  //  this course is priced at all".
  //
  //  It is not the same number as `graded`, and this view used to publish only
  //  one field called `possible` carrying the ATTEMPTED sum. A consumer reading
  //  it to ask how much of the course is priced got the attempted total instead,
  //  which equals the graded total by construction and therefore always looked
  //  complete. docs/gradebook-contract.md: three denominators, do not collapse
  //  them.
  let coursePossible = 0, itemsTotal = 0;
  for (const it of itemList) {
    if (!it.graded_kind) continue;
    itemsTotal += 1;
    if (it.denominator != null) coursePossible += it.denominator;
  }
  coursePossible = Math.round(coursePossible * 100) / 100;
  ...
  const students = roster.map((s, idx) => {
    ...
    for (const it of itemList) {
      // `attempts` has no unit column, so that map stays lesson keyed. The
      // courses that write to it (CSA, networking) have no lesson name repeated
      // across units, so nothing collides there.
      const att = attemptCell.get(`${s.id}|${it.lesson_id}|${it.activity}`);
      const pr = progCell.get(`${s.id}|${it.unit}|${it.lesson_id}|${it.activity}`);
      let cell = null;

      if (att && att.possible > 0 && isGradedAct(it.activity)) {
        const p = pct(att.earned, att.possible);
        // Real attempted points WIN over any authored total. The attempt already
        // records how many points were earned and out of how many, which is the
        // page's own count of its own questions. An authored value is a guess
        // about that page, so it must never rescale a real pair.
        const possible = att.possible;
        const earned = Math.round(att.earned * 100) / 100;
        cell = {
          pct: p, source: 'attempt', items: att.items, tries: att.tries,
          earned, possible,
          denominator_source: 'observed',
        ...
        if (authored != null) {
          cell.earned = pointsFromRatio(p / 100, authored);
          cell.possible = authored;
          cell.denominator_source = 'authored';
          // A priced cell has real marks and must join the points total, exactly
          // as the attempts branch above does. Without this every cyber and CSP
          // class summed to 0 out of 0, because only the attempts path ever fed
          // these, and the row total printed "0/0" under a percentage.
          earnedSum += cell.earned; possibleSum += cell.possible;
        }
        pctSum += p; pctCount += 1;
        ...
        // A GRADED activity finished with no number behind it is not the same
        // fact as a lesson visit, and it must not render alike. The teacher view
        // has said so since the contract landed; this one lumped both into
        // "done", which reads as done and fine, and that is how a week of AP
        // Cyber Ex2, Lab and Quiz completions looked healthy while their pages
        // posted no score at all. public/teachers.html has drawn the X off this
        // flag the whole time: the flag was simply never set here, so the mark
        // and its legend were dead code on the operator page.
        //
        // Nothing read-time can recover it. The number was never sent.
        if (it.graded_kind) { cell.score_missing = true; scoreMissing += 1; }
      }
      ...
      if (cell) {
        ...
        if (cell.pct != null) {
          ...
          const la = lessonAgg.get(it.lesson_id) || { sum: 0, n: 0, earned: 0, possible: 0, priced: 0 };
          la.sum += cell.pct; la.n += 1;
          // Carry the POINTS as well as the percentage, so the compact view can
          // show a lesson as marks over marks like every other view does. Only
          // priced cells contribute: a column nobody has assigned points to has
          // no marks to add, and inventing some is what this whole contract
          // exists to prevent.
          if (cell.earned != null && cell.possible != null && cell.possible > 0) {
            la.earned += cell.earned; la.possible += cell.possible; la.priced += 1;
          }
    ...
    for (const [lesson, la] of lessonAgg) {
      // earned/possible are null unless EVERY graded item in the lesson is
      // priced. A partial sum would read as a whole one and quietly understate
      // the lesson: "6/8" when the lesson also holds an unpriced quiz is a
      // smaller, wronger claim than showing the percentage and saying so.
      const whole = la.priced === la.n && la.possible > 0;
      // AND WHERE THERE IS A PAIR, THE PERCENT IS THAT PAIR.
      //
      // This averaged the item percentages even when it was printing marks
      // beside them, so a lesson holding a 5 mark quiz at 100 and a 25 mark
      // project at 20 rendered "10 / 30" with "60%" next to it. Ten marks out of
      // thirty is 33. The pair was right, the percent was a mean, and the two
      // were never required to agree: the same shape as the 483% cell, one level
      // up, and the same rule board 85 states for a student row.
      //
      // A lesson with an unpriced item in it keeps the mean, because it shows no
      // pair for the mean to contradict and there is no honest points answer for
      // work nobody has assigned marks to. `basis` says which of the two a
      // reader is looking at.
      const points = whole ? gradePct(la.earned, la.possible) : null;
      const p = points != null ? round(points) : round(la.sum / la.n);
      cells[lesson] = {
        pct: p, basis: points != null ? 'points' : 'percent',
        items: la.n, passed: p >= threshold,
        earned: whole ? Math.round(la.earned * 100) / 100 : null,
        possible: whole ? Math.round(la.possible * 100) / 100 : null,
        items_priced: la.priced,
      };
    }

    // The grade is POINTS over POINTS, the same rule lib/gradebook-contract.js
    // applies, so the operator view and the teacher view cannot disagree about a
    // student's grade.
    //
    // This used to average PERCENTAGES, and mixed a points percentage with a
    // mean of percentages when both existed. That is the defect the contract was
    // written to remove, and it was still live here: a cyber student on
    // 0/7, 7/8 and 5/5 plus two unpriced zeroes read 38 percent on this page and
    // 60 percent on the teacher's, because the mean counted two columns nobody
    // had assigned points to as if they were whole assignments worth the same as
    // the quiz. Twelve marks out of twenty is 60.
    //
    // Unpriced work still cannot join a points sum, so it is excluded here and
    // shown in its own cell, which is what items_percent_only reports.
    let overallPct = null, basis = 'none';
    const overallPoints = gradePct(earnedSum, possibleSum);
    if (overallPoints != null) { overallPct = round(overallPoints); basis = 'points'; }
    else if (pctCount > 0) {
      // Nothing in this class has points assigned at all, so there is no points
      // total to divide. The mean of the percentages is the only number
      // available; it is labelled 'percent' so a reader can tell.
      overallPct = round(pctSum / pctCount); basis = 'percent';
    }
    ...
    return {
      ...
      overall: {
        ...
        earned: Math.round(earnedSum * 100) / 100,
        // `graded` is the sum over ATTEMPTED work and is what pct divides by.
        // `possible` is the whole course. Same names, same meanings, same
        // arithmetic as GET /class/:id/gradebook/as-teacher, so an operator
        // reading one and a teacher reading the other cannot come away with
        // different denominators for the same class.
        graded: Math.round(possibleSum * 100) / 100,
        possible: coursePossible,
        items_total: itemsTotal,
        // Graded work this student COMPLETED whose score never arrived. Never a
        // grade component: there is no number to count. It is the tally of cells
        // a teacher must not read as healthy.
        items_score_missing: scoreMissing,
      },
    };
  });

  // ── Per-lesson class averages ─────────────────────────────────────────────
  //  Points over points, for the same reason a student's grade is: a mean of
  //  percentages lets one student's 5 mark quiz weigh exactly as much as another
  //  student's 25 mark project. That is the defect board 85 names, one level up
  //  from the student row where it was fixed, and it is the number a teacher
  //  actually reads off the bottom of the column.
  //
  //  The mean of percentages survives only where the column has no marks at all,
  //  labelled so a reader can tell which number they are looking at. That is the
  //  same concession `basis` makes on a student row, for the same reason: where
  //  nobody has priced the work, the mean is the only number there is.
  for (const l of lessonList) {
    let e = 0, g = 0, sum = 0, n = 0, priced = 0, pas = 0;
    for (const st of students) {
      const c = st.cells[l.lesson_id];
      if (!c || c.pct == null) continue;
      sum += c.pct; n += 1;
      if (c.passed) pas += 1;
      if (c.earned != null && c.possible != null && c.possible > 0) {
        e += c.earned; g += c.possible; priced += 1;
      }
    }
    const points = gradePct(e, g);
    l.class_avg_pct = points != null ? round(points) : (n ? round(sum / n) : null);
    l.class_avg_basis = points != null ? 'points' : (n ? 'percent' : 'none');
    l.class_earned = points != null ? Math.round(e * 100) / 100 : null;
    l.class_graded = points != null ? Math.round(g * 100) / 100 : null;
    l.graded_students = n;
    l.priced_students = priced;
    l.passed_students = pas;
  }

  // ── Per-item class averages + fill rate ───────────────────────────────────
  //  Same rule as the lesson rollup above, and the same rule lib/gradebook-
  //  contract.js applies to its own columns.
  for (const it of itemList) {
    let e = 0, g = 0, sum = 0, n = 0, done = 0, priced = 0;
    for (const st of students) {
    ...
    const points = gradePct(e, g);
    it.class_avg_pct = points != null ? round(points) : (n ? round(sum / n) : null);
    it.class_avg_basis = points != null ? 'points' : (n ? 'percent' : 'none');
    it.class_earned = points != null ? Math.round(e * 100) / 100 : null;
    it.class_graded = points != null ? Math.round(g * 100) / 100 : null;
    it.scored_students = n;
    it.priced_students = priced;
  ...
  for (const it of itemList) {
    const c = covMap.get(it.activity) || {
      ...
      scored_cells: 0, completed_cells: 0, possible_cells: 0,
      earned: 0, graded: 0, sum: 0, pct_cells: 0,
    ...
    // Points where the column has them. A mean of column averages was a mean
    // of means: it weighted a 5 mark quiz and a 25 mark project equally, then
    // weighted each column by how many students happened to sit it.
    if (it.class_graded != null) { c.earned += it.class_earned; c.graded += it.class_graded; }
    else if (it.class_avg_pct != null) {
      c.sum += it.class_avg_pct * it.scored_students;
      c.pct_cells += it.scored_students;
    }
    covMap.set(it.activity, c);
  }
  const activity_coverage = [...covMap.values()].map((c) => {
   const points = gradePct(c.earned, c.graded);
   return {
    activity: c.activity,
    label: c.label,
    ...
    avg_pct: points != null ? round(points) : (c.pct_cells ? round(c.sum / c.pct_cells) : null),
    avg_basis: points != null ? 'points' : (c.pct_cells ? 'percent' : 'none'),
    earned: points != null ? Math.round(c.earned * 100) / 100 : null,
    graded: points != null ? Math.round(c.graded * 100) / 100 : null,
  ...
  // The class percentage is points over points, so it agrees with the rows it
  // summarizes. A mean of student percentages gave a class of two, one on 1/1
  // and one on 12/40, a class average of 65 when the class had earned 13 marks
  // out of 41.
  const classEarned = Math.round(withScores.reduce((n, s) => n + s.overall.earned, 0) * 100) / 100;
  const classGraded = Math.round(withScores.reduce((n, s) => n + s.overall.graded, 0) * 100) / 100;
  const classPoints = gradePct(classEarned, classGraded);
  return {
    class: {
    ...
    summary: {
      ...
      class_avg_pct: classPoints != null ? round(classPoints)
        : (withScores.length
          ? round(withScores.reduce((n, s) => n + s.overall.pct, 0) / withScores.length)
          : null),
      class_avg_basis: classPoints != null ? 'points' : (withScores.length ? 'percent' : 'none'),
      earned: classEarned,
      graded: classGraded,
      possible: coursePossible,
      items_total: itemsTotal,
