...
function buildGradebook(classKey, opts) {
  ...
  const ensureItem = (lesson, activity, unit, expected) => {
    ...
    if (!items.has(key)) {
      items.set(key, {
        ...
        expected: !!expected, observed: false, graded_kind: !VISIT_ACTS.has(activity),
        in_manifest: false,
      });
    }
  ...
  const students = roster.map((s, idx) => {
    ...
    for (const it of itemList) {
      ...
      if (att && att.possible > 0) {
      ...
      } else if (pr && pr.score != null && !VISIT_ACTS.has(it.activity)) {
        ...
        pctSum += p; pctCount += 1;
      } else if (pr && pr.completed) {
        // Completed but carries no number: lessons, and any activity the pages
        // mark done without reporting a score.
        cell = { pct: null, source: 'done', completed: true, passed: null };
      }
