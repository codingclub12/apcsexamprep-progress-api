...
const SCHEMA_SQL = `
  ...
  -- ── PHASE 2 ──────────────────────────────────────────────────────────────
  -- Deterministic check results: nightly production smoke, CI, /api/health, and
  -- a scheduled link-check on known-critical URLs. ONE ROW PER CHECK, not per
  -- run: a suite failing five days running is one row aging with first_failed_at
  -- pinned to the first failure, which is what makes the task age rather than
  -- multiply. Nothing fuzzy ever writes here; a noise queue gets ignored, and
  -- then so does everything next to it.
  CREATE TABLE IF NOT EXISTS checks (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    source          TEXT NOT NULL,     -- smoke | ci | health | linkcheck
    check_id        TEXT NOT NULL,     -- suite name, workflow job, url, ...
    state           TEXT NOT NULL,     -- pass | fail
    detail          TEXT,              -- last message, truncated
    first_failed_at TEXT,              -- pinned at the FIRST fail, cleared on green
    last_seen_at    TEXT DEFAULT (datetime('now')),
    last_pass_at    TEXT,
    fail_count      INTEGER NOT NULL DEFAULT 0,
    task_id         INTEGER REFERENCES tasks(id) ON DELETE SET NULL
  );
  CREATE UNIQUE INDEX IF NOT EXISTS uidx_checks_fingerprint ON checks(source, check_id);
  CREATE INDEX IF NOT EXISTS idx_checks_state ON checks(state);

  -- Daily aggregates ONLY. Never raw events. This runs on a 1 vCPU / 1GB box
  -- that has already produced a $169 memory-leak spike, and an unbounded metrics
  -- table is how that happens twice. Retention is 400 days, enforced by a
  -- scheduled delete in lib/command-checks.js that runs at most once a day.
  CREATE TABLE IF NOT EXISTS metrics_daily (
    date        TEXT NOT NULL,          -- 'YYYY-MM-DD'
    source      TEXT NOT NULL,          -- gradebook | pipeline | gsc | ga4 | raptive
    metric      TEXT NOT NULL,
    value       REAL NOT NULL,
    dimension   TEXT NOT NULL DEFAULT '',  -- '' rather than NULL so the PK works
