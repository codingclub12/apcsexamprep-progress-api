...
const { esc, escAttr, SITE, COURSE_URL, PROJECTS_URL } = require('./intro-java-page');
...
${meta.untaught.map((u) => `<tr><td><code>${esc(u.call)}</code></td><td>${esc(u.why)}</td></tr>`).join('\n')}
  ...
  // Previous, up, next. The middle link goes to the project LIST on the course
  // hub rather than the top of the hub, because that is the shelf the student
  // picked this off and the one they want to be standing in front of again.
  const nav = [
    o.prev ? `<a rel="prev" href="${escAttr(`${SITE}/pages/${handleFor(o.prev)}`)}">Previous: ${esc(o.prev.title)}</a>` : '<span></span>',
    `<a rel="up" href="${escAttr(PROJECTS_URL)}">All projects</a>`,
    o.next ? `<a rel="next" href="${escAttr(`${SITE}/pages/${handleFor(o.next)}`)}">Next: ${esc(o.next.title)}</a>` : '<span></span>',
  ].join('\n');
  ...
  const body = [
    ...
    `<p class="ij-meta">Greenfoot project ${esc(String(project.n))} &middot; `
      + `<a href="${escAttr(PROJECTS_URL)}">all projects</a> &middot; `
