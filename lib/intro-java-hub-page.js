...
const { handleFor: projectHandleFor } = require('./intro-java-project-page');
...
const { COURSE_HANDLE, COURSE_SLUG, unitHandle, PROJECTS_ANCHOR } = require('./intro-java-page');
...
// COURSE_SLUG and unitHandle moved to lib/intro-java-page.js. Unit hubs are NEW
// pages with no legacy slug to inherit, so they keep the clean prefix, and they
// are deliberately not derived from COURSE_HANDLE: that would have turned every
// unit into `greenfoot-basics-beginner-greenfoot-projects-and-tutorials-unit-1`.
// They live one module down now because lesson pages link up to their own unit
// hub, and importing this file from that one would be a cycle. Still re-exported
// from here, which is where callers have always found them.
  ...
  faq: [
    ...
    },
    {
      ...
      a: 'Roughly 85 percent of the points. Concept checks, fill-in-the-code exercises and short '
        + 'quizzes are graded automatically; the six game projects are scored by the teacher '
        + 'against a rubric.',
    },
    {
...
const HOW_IT_IS_TAUGHT = [
  ...
  },
  {
    ...
    p: 'Students fill specific gaps in working code. It teaches the syntax without demanding a '
      + 'beginner produce a whole class from nothing, and it is graded instantly.',
  },
  {
...
<p>${esc(f.a)}</p>
  ...
  // The project library this page already carried.
  //
  // These used to be bare external links straight to YouTube and Drive, which
  // sent every student who wanted a tutorial off the site to get it. Each
  // project now has its own page carrying the video embedded plus the method
  // list, so the hub links THERE and the visit stays here. The starter files
  // and the video are both on that page, one click further in.
  //
  // Array Racing has neither a video nor files, so it has no page. It keeps
  // the coming-soon line rather than linking somewhere empty.
  const internal = (handle, label) =>
    `<a class="ij-proj-link" href="${escAttr(`${SITE}/pages/${handle}`)}">${esc(label)}</a>`;
  const projectsHtml = PROJECTS.map((p) => {
    const hasPage = (p.videos && p.videos.length) || p.files;
    const linkRow = hasPage
      ? `<p class="ij-proj-links">${internal(projectHandleFor(p), 'Open the project')}</p>`
...
${paras('What you score by hand is the six game projects, against a rubric. That is deliberate: a '
  ...
  + 'pretended otherwise would be grading a checkbox rather than a game.')}
</div>`,

    // The anchor every project page links back to. Without it "back to the
    // projects" lands at the top of a long hub page and the student has to hunt
    // for the list they were already standing in.
    `<h2 id="${PROJECTS_ANCHOR}">Greenfoot project tutorials</h2>`,
    paras(
...
// need, so each says what a student can DO afterward rather than listing topics.
const UNIT_COPY = {
  'unit-1': {
    blurb: 'The orientation unit. Students meet scenarios, worlds and actors, learn what a class '
      + 'and an object actually are, and write their first behavior inside act(). Nothing here '
