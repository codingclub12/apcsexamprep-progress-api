...
function readHarness(file) {
  ...
  //  Every harness this was written for writes its sabotage into a tracked file
  //  and restores it on the way out, which is the whole reason a leak is
  //  possible. A harness can also damage a STRING and hand it to the gate
  //  function, touching no file at all: smoke/csa-ek-thin-mutation.js does, and
  //  it cannot leak a mutation because it never writes one.
  //
  //  Such a harness has no find/repl pairs and no target files, so both guards
  //  below fired on it and the suite went red on a harness that is strictly
  //  safer than the ones it was built for.
  //
  //  THE DECLARATION IS CHECKED, NOT TRUSTED. Saying MUTATION_LEAK = 'in-memory'
  //  buys nothing on its own; a harness that says it and then writes a file
  //  would be the worst case here, because it would be skipped. So the claim is
  //  verified against the source: any fs call that can put bytes on disk, and
  //  the declaration is refused. A marker a harness can assert about itself
  //  without evidence is a marker that will eventually be wrong.
  if (/\bMUTATION_LEAK\s*=\s*'in-memory'/.test(src)) {
    //  COMMENTS STRIPPED FIRST. The comment explaining this rule names the very
    //  call the rule forbids, and the first version of the check read it and
    //  refused the harness that had just been written to satisfy it. A sentence
    //  about fs.writeFileSync is not a call to it, the same way an instruction
    //  comment containing a literal <div> is not markup; lib/cyber-page-gate.js
    //  learned that one the same way.
    const code = src.replace(/\/\*[\s\S]*?\*\//g, ' ').replace(/(^|[^:])\/\/[^\n]*/g, '$1');
    const writes = code.match(/\bfs\s*\.\s*(write|append|cp|copy|rm|unlink|mkdir|rename|truncate)\w*/g) || [];
    if (writes.length) {
      throw new Error(`${rel}: declares MUTATION_LEAK = 'in-memory' and calls `
        + `${[...new Set(writes)].join(', ')}. A harness that writes to disk can leak a mutation, `
        + 'whatever it says about itself.');
    }
    return { harness: rel, pairs: [], targets: [], inMemory: true };
  }

  if (!pairs.length) {
    throw new Error(`${rel}: no find/repl pair found. A harness contributing nothing reads `
      + 'exactly like one this has stopped understanding. If it damages a string rather than a '
      + "file, declare MUTATION_LEAK = 'in-memory' and write no files.");
  }
