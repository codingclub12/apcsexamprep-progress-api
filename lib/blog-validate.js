...
function validatePost(post, seenHandles) {
  ...
  if (h1s.length !== 1) {
    bad(`body has ${h1s.length} h1 tags, needs exactly 1 (this theme renders no h1 for articles)`);
  }
  ...
  if (m.targetKeyword) {
    ...
    const h1Text = ((body.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i) || [])[1] || '').toLowerCase();
    const plain = stripTags(prosePartOnly(body)).toLowerCase();
    const opening = plain.slice(0, 900);
    if (!h1Text.includes(kw)) bad(`target keyword "${m.targetKeyword}" is not in the h1`);
...
module.exports = { validatePost, validateBatch, LIMITS, words };
