...
const COURSES = ['ap-csa', 'ap-csp', 'ap-cybersecurity', 'ap-networking', 'intro-java', 'solo'];
...
// sinceDays limits the ACTIVITY metrics (completions, sessions, attempts, and
// everything derived from them: by_course/by_teacher/geography completions +
// sessions, funnel, devices, acquisition, hardest items, completions-by-unit) to
// a recent window. 0 / falsy = all time. Structural counts (classes, students,
// teachers, geography roster), the recency cohorts, and the 30-day trend series
// are always as-of-now and ignore the window. Allowed windows: 7, 30, 90, 365.
const RANGE_DAYS = new Set([7, 30, 90, 365]);
function computeAnalytics(sinceDays) {
  const generated_at = new Date().toISOString();
  const days = RANGE_DAYS.has(Number(sinceDays)) ? Number(sinceDays) : 0;
  // days is validated to a small allow-list, so interpolating it is safe.
  const sSess = days ? ` AND started_at >= datetime('now','-${days} days')` : '';
  const sComp = days ? ` AND COALESCE(completed_at, updated_at) >= datetime('now','-${days} days')` : '';
  const sAtt = days ? ` AND created_at >= datetime('now','-${days} days')` : '';
  ...
  if (!realIds.length) {
    return {
      generated_at, empty: true, range: { days },
      ...
      acquisition: { new_vs_returning: { new: 0, returning: 0, total: 0, returning_pct: 0 }, by_channel: [], top_referrers: [] },
  ...
  // Windowed completions per class: the completions figure everywhere (by_course,
  // by_teacher, geography) respects the date range, while student/class/teacher
  // counts stay as-of-now.
  const compByClass = new Map(db.prepare(
    `SELECT class_id, COUNT(*) AS n FROM progress
       WHERE completed = 1${sComp} AND class_id IN (${inReal}) GROUP BY class_id`
  ).all(...realIds).map((r) => [r.class_id, r.n]));
  const compOf = (id) => compByClass.get(id) || 0;
  ...
  const sessByClass = new Map(db.prepare(
    `SELECT class_id, COUNT(*) AS sessions, COALESCE(SUM(active_seconds),0) AS act
       FROM sessions WHERE class_id IN (${inReal})${sSess} GROUP BY class_id`
  ...
  const by_teacher = Object.values(teacherAgg).map((a) => {
    const es = dir.expectedStart(a.teacher_email);
    return {
      teacher: a.teacher_name, email: a.teacher_email,
      state: es.state, // 2-letter state from the email domain, or null
      expected_start: es.date, // 'Aug 13' etc., or null if unknown
      expected_start_approx: es.approx, // true = state-level estimate, false = curated district date
  ...
  const unitRows = db.prepare(`
    ...
    FROM progress WHERE completed = 1${sComp} AND class_id IN (${inReal})
  ...
  const visited = one(`SELECT COUNT(DISTINCT student_id) n FROM progress WHERE completed = 1${sComp} AND class_id IN (${inReal})`);
  const attempted = one(`SELECT COUNT(DISTINCT student_id) n FROM attempts WHERE class_id IN (${inReal})${sAtt}`);
  const passed = one(`SELECT COUNT(DISTINCT student_id) n FROM attempts WHERE passed = 1${sAtt} AND class_id IN (${inReal})`);
  const funnel = [
  ...
  const uaRows = db.prepare(
    `SELECT ua, COUNT(*) AS sessions, COALESCE(SUM(active_seconds),0) AS act
       FROM sessions WHERE class_id IN (${inReal})${sSess} GROUP BY ua`
  ).all(...realIds);
  ...
  // ── acquisition: new vs returning users, by first-touch channel ────────────
  //  A user's channel is their ENTRY channel (earliest session). Returning = the
  //  user has sessions on 2+ distinct days. First-touch attribution means each
  //  user counts once, so the new/returning split per channel never overlaps.
  const perStudent = db.prepare(`
    SELECT s.student_id,
           COUNT(DISTINCT DATE(s.started_at)) AS days,
           COUNT(*) AS sessions,
           COALESCE(SUM(s.active_seconds), 0) AS act,
           (SELECT COALESCE(s2.channel, 'Direct') FROM sessions s2
              WHERE s2.student_id = s.student_id ORDER BY s2.started_at ASC, s2.id ASC LIMIT 1) AS first_channel
    FROM sessions s WHERE s.class_id IN (${inReal})${days ? ` AND s.started_at >= datetime('now','-${days} days')` : ''}
  ...
  for (const r of perStudent) {
    const returning = r.days >= 2;
    if (returning) returningUsers += 1; else newUsers += 1;
    const ch = r.first_channel || 'Direct';
    const a = chanAgg[ch] || (chanAgg[ch] = { channel: ch, users: 0, new_users: 0, returning_users: 0, sessions: 0, active_minutes: 0 });
    a.users += 1;
    if (returning) a.returning_users += 1; else a.new_users += 1;
    a.sessions += r.sessions;
    a.active_minutes += minutes(r.act);
  }
  const totalUsers = newUsers + returningUsers;
  const acquisition = {
    new_vs_returning: {
      new: newUsers, returning: returningUsers, total: totalUsers,
      returning_pct: pct(returningUsers, totalUsers),
    },
    by_channel: Object.values(chanAgg).sort((a, b) => b.users - a.users),
    top_referrers: db.prepare(
      `SELECT referrer_host AS host, COUNT(*) AS sessions FROM sessions
        WHERE class_id IN (${inReal})${sSess} AND referrer_host IS NOT NULL AND referrer_host != ''
        GROUP BY referrer_host ORDER BY sessions DESC LIMIT 10`
    ).all(...realIds),
  };

  // ── trends: last 30 days, one point per day (always 30d; ignores the range) ─
  const trendDays = [];
  const today = new Date();
  for (let i = 29; i >= 0; i--) {
    const d = new Date(today.getFullYear(), today.getMonth(), today.getDate() - i);
    trendDays.push(d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0') + '-' + String(d.getDate()).padStart(2, '0'));
  }
  ...
  const hardest_items = db.prepare(`
    ...
    FROM attempts WHERE class_id IN (${inReal})${sAtt}
