...
function bootstrapScript(opts) {
  ...
  const fallbackHtml = '<div class="apcs-offline">'
    ...
    //  OVERRIDABLE, because it stopped being true for every activity. Analysis
    //  activities send their responses to be graded and discarded, so a page
    //  that claims nothing is ever sent would be lying to a student about where
    //  their words go. Every existing caller omits the option and gets exactly
    //  the sentence it always got.
    + '<p>' + (opts.privacyLine
      || 'Nothing you have written is affected. These are self-scored and nothing '
