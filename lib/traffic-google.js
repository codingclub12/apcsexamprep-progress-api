...
//  Country, device and channel are NOT fetched per day. They have tens or
//  hundreds of distinct values, not thousands, so a single ranged request with a
//  high row limit returns every day in full and costs one call instead of one
//  per day.
//
//  That is the same shape as the bug fixed earlier, so the distinction matters:
//  the row limit was harmful for page and query because it BOUND, silently
//  dropping the older half of the range. Here it should not bind. "Should not"
//  is not a guarantee, so truncation is DETECTED rather than assumed away: if a
//  response comes back exactly at the limit, the caller is told, instead of
//  quietly serving a starved breakdown that looks like a real decline.
const WIDE_ROW_LIMIT = 25000;

function truncationNote(rows, limit, label) {
  return rows.length >= limit
    ? `${label} hit the ${limit}-row limit and is incomplete; narrow the date range`
    : null;
}
...
async function fetchGa4({ startDate, endDate } = {}) {
  ...
  // Low-cardinality breakdowns over the whole range in one request each.
  // Country, device and channel have tens or hundreds of values, not thousands,
  // so the row limit should not bind. Truncation is detected, not assumed away.
  const notes = [];
  const WIDE = [
    ['country', 'country'],
  ...
  for (const [gaDim, ns] of WIDE) {
    if (!covered) break;
    const res = await runGa4Report(token, propertyId, {
      dateRanges,
      dimensions: [{ name: 'date' }, { name: gaDim }],
      metrics: [{ name: 'sessions' }, { name: 'screenPageViews' }, { name: 'totalUsers' }],
      limit: WIDE_ROW_LIMIT,
    });
    const rows = res.rows || [];
    const note = truncationNote(rows, WIDE_ROW_LIMIT, 'ga4 ' + ns);
    if (note) notes.push(note);
    for (const row of rows) {
      const date = isoDate(row.dimensionValues[0].value);
      const value = row.dimensionValues[1].value;
      readings.push({ date, source: 'ga4', metric: 'sessions', value: Number(row.metricValues[0].value),
        dimension_namespace: ns, dimension_value: value });
      readings.push({ date, source: 'ga4', metric: 'pageviews', value: Number(row.metricValues[1].value),
        dimension_namespace: ns, dimension_value: value });
      readings.push({ date, source: 'ga4', metric: 'users', value: Number(row.metricValues[2].value),
        dimension_namespace: ns, dimension_value: value });
    }
  }
...
async function fetchGsc({ startDate, endDate } = {}) {
  ...
  // Country and device over the whole range in one request each, same reasoning
  // as GA4: low cardinality, so the limit should not bind, and truncation is
  // detected rather than trusted not to happen.
  const notes = [];
  for (const dim of ['country', 'device']) {
    if (!covered) break;
    const res = await runGscQuery(token, siteUrl, {
      startDate: covered.from, endDate: covered.to, dimensions: ['date', dim], rowLimit: WIDE_ROW_LIMIT,
    });
    const rows = res.rows || [];
    const note = truncationNote(rows, WIDE_ROW_LIMIT, 'gsc ' + dim);
    if (note) notes.push(note);
    for (const row of rows) {
      const date = row.keys[0];
      const value = row.keys[1];
      push(date, 'clicks', row.clicks, dim, value);
      push(date, 'impressions', row.impressions, dim, value);
      if (row.position > 0) push(date, 'position', row.position, dim, value);
    }
  }
