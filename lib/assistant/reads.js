...
// ─────────────────────────────────────────────────────────────────────────────
const db = require('../../db');
const { resolveGate, pickGateRow } = require('../activity-gate');
...
`);
// Every gate row for the class, narrowed in JS by pickGateRow. A gate may sit at
// unit, lesson or activity scope, so an equality match on lesson and
// activity_type would report a unit-wide lock as "open" and send a teacher
// hunting for a lock that is plainly in their own list. One query per call
// rather than one per activity, which also drops this loop off the N+1 path.
const stGateRows = db.prepare(`
  SELECT unit, lesson, activity_type, open FROM activity_gates
  WHERE class_id = ? AND course = ?
`);
const gatesForUnit = (rows, unit) => rows.filter((r) => r.unit === unit);
...
function getGateState(teacherId, classCode, opts = {}) {
  ...
  const gateRows = gcls ? stGateRows.all(gcls.id, course) : [];
  const activities = rows.map((a) => {
    const row = gcls ? pickGateRow(gatesForUnit(gateRows, a.unit), a.lesson, a.activity_type) : null;
...
function getMyGates(studentId, opts = {}) {
  ...
  const gateRows = stGateRows.all(cls.class_id, course);
  const activities = rows.map((a) => {
    const row = pickGateRow(gatesForUnit(gateRows, a.unit), a.lesson, a.activity_type);
    const g = resolveGate(row, gcls, a.activity_type);
    return {
      unit: a.unit,
      lesson: a.lesson,
