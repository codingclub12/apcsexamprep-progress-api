...
function severityFor(category, role) {
  ...
  // broken on an assessment product. Immediate regardless of role, because the
  // role is not what makes it urgent. It cannot be raised by a client; see
  // INTERNAL_CATEGORIES above.
...
function store(input) {
  ...
  const row = {
    id,
    // NULL for a Phase 0 report, which has no chat session because Phase 0 has
    // no chat. Phase 2 passes one, so a chat-raised escalation links back to the
...
module.exports = {
  notifyStatus,
  CATEGORIES, CATEGORY_SET, INTERNAL_CATEGORIES, ALL_CATEGORIES, LIMITS, MAX_REPORTS_PER_DAY, MAX_NEW_TASKS_PER_DAY,
