...
// request.
// `model` overrides ASSISTANT_MODEL for one call. It exists for the junk filter,
// which labels a sentence real, vague or junk and has no business spending Opus
// tokens to do it. Deliberately a per-call argument rather than a second env
// variable: the caller that wants a cheap model knows it at the call site, and a
// global would let a cheap model leak into the support replies.
function buildRequest({ system, messages, maxTokens: mt, model: modelOverride }) {
  const body = {
...
async function complete({ system, messages, maxTokens: mt, model: modelOverride }) {
  if (!configured()) return { ok: false, reason: 'unconfigured', text: null, usage: null };

  const body = buildRequest({ system, messages, maxTokens: mt, model: modelOverride });
  let Anthropic;
  try {
    if (!_sdk) _sdk = require('@anthropic-ai/sdk');
