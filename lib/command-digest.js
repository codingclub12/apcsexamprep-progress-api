...
const checksLib = require('./command-checks');
...
function summary(t, baseUrl) {
  return {
    ...
    // Deep link straight to this row on the board, panel open, Verify in front
    // of you. `verified` is still cookie-auth only and a human still clicks it:
    // this changes the number of clicks, never who is allowed to make them.
    //
    // The reason it is worth a field at all is that the friction was real. The
    // owner has to open the board, remember which of 27 rows the agent meant,
    // and find it under filters that default to hiding exactly the tasks
    // awaiting verification: owner "tanner" hides agent-owned work, and closed
    // is off by default so anything done is not even fetched. A control people
    // route around because it is tedious stops being a control.
    verify_url: `${baseUrl}/admin/command#t${t.id}`,
  };
}
...
function buildDigest({ baseUrl = '', includePii = false } = {}) {
  ...
  return {
    ...
    stuck,
    // Phase 1 free metrics (same SQLite, same process) plus Phase 2 check state.
    // Wrapped because `checks` is a Phase 2 table and an older database that
    // predates it must still serve the whole Phase 1 digest.
    health: Object.assign({}, store.healthSnapshot(), {
      checks: (() => {
        try { return checksLib.checksSummary(); } catch (_) { return null; }
      })(),
    }),
