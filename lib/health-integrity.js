...
// ─────────────────────────────────────────────────────────────────────────────
//  THIRD BLOCK: a column priced at one number while students are being served
//  another.
//
//  ── WHY THIS EXISTS ─────────────────────────────────────────────────────────
//  On 2026-09-07 a teacher wrote in about her gradebook: "1.1 Ex 1 is out of 7
//  but scores are out of 14? Ex 2 is out of 8, but scores show out of 15?" Both
//  were real, and they had different causes: Exercise 1 was one run reported by
//  two writers and summed, Exercise 2 was a page rebuilt from 8 questions to 15
//  while the authored price stayed at 8.
//
//  One check catches both, because both have the same signature: the number the
//  column header prints disagrees with the number the students' own submissions
//  carried. Nothing looked at that, and the detector was an email.
//
//  ── WHAT IT COMPARES, AND THE MISTAKE THE FIRST VERSION MADE ────────────────
//  The authored price in course_denominators against what the LEDGER SAYS THE
//  PAGE IS OUT OF. Getting that second number right is the whole check, and the
//  first version got it wrong in a way that made the alarm mostly noise.
//
//  It compared against each student's own summed max_points. That is a
//  denominator only where the page reports its whole run in ONE row, under the
//  reserved 'score' item that assets/apcs-score-reporter.js writes through
//  APCS_saveLessonScore. Where a page reports one row PER QUESTION, a student's
//  sum is how many questions they answered before they stopped, which is
//  progress, not a total.
//
//  Measured on production 2026-09-08, after the six real hits were fixed, EVERY
//  remaining row was that mistake:
//
//    ap-csp bi-1 collaboration quiz   priced 6, observed 1 and 3 and 5
//
//  One column, three rows, because three students answered 1, 3 and 5 of 6 and
//  stopped. Nothing is wrong with that column. Fourteen of the seventeen rows
//  were this, and an alarm whose every row is healthy work is one people learn
//  to skip, which is the failure the reporter block next door was rewritten to
//  avoid.
//
//  So two rules, and each removes a different class:
//
//    carrier only   a per-question column cannot state a denominator, so it is
//                   not compared at all. Silence is the honest answer there.
//    the MAXIMUM    across students rather than each student's own sum. One
//                   student who abandoned a carrier-reporting page halfway must
//                   not make a correct price look wrong.
//
//  A disagreement that survives both is not automatically the price being wrong.
//  A page rebuilt today is served to a student tomorrow, so the honest reading
//  is "these two numbers should be reconciled", which is why the row carries
//  both and the count of students who recorded the maximum.
//
//  ── WHY IT IS SAFE TO MAKE PUBLIC ───────────────────────────────────────────
//  Same terms as the two blocks above. A row is a course, a lesson, an activity,
//  two totals and a count of students. "ap-cybersecurity 1.1 exercise-2 is
//  priced 8 and 4 students were served 15" names a PAGE and a price. Nothing in
//  it narrows to a person.
//
//  NEVER THROWS. /api/health answering 200 is load-bearing.
// ─────────────────────────────────────────────────────────────────────────────

// A price is a property of a page and moves when somebody edits one, so this
// sits on the same longer cache as its neighbour rather than the seed TTL.
...
function priceStatement() {
  if (!priceStmt) {
    //  The carrier restriction is stated ONCE, in the CTE. An earlier draft
    //  wrote it in two subqueries that joined to each other, and the deploy
    //  gate caught that immediately: breaking either one changed no behaviour,
    //  so no test could tell whether either worked. Two guards that cover for
    //  each other are indistinguishable from one guard and one dead line.
    priceStmt = db.prepare(`
      WITH carrier AS (
        SELECT course, unit, lesson, activity_type, student_id,
               MAX(max_points) AS possible
        FROM score_events
        WHERE item = '${REPORTER_TOTAL_ITEM}'
        GROUP BY course, unit, lesson, activity_type, student_id
      ),
      topped AS (
        SELECT course, unit, lesson, activity_type, student_id, possible,
               MAX(possible) OVER (
                 PARTITION BY course, unit, lesson, activity_type
               ) AS top
        FROM carrier
      )
      SELECT t.course, t.unit, t.lesson, t.activity_type,
             d.possible AS authored,
             t.top      AS observed,
             COUNT(*)   AS students
      FROM topped t
      JOIN course_denominators d
        ON d.course = t.course AND d.lesson = t.lesson AND d.activity_type = t.activity_type
      WHERE t.possible = t.top
        AND t.top > 0
        AND t.activity_type NOT IN ('lesson', 'visit', 'page', 'read')
        AND d.possible <> t.top
      GROUP BY t.course, t.unit, t.lesson, t.activity_type, d.possible, t.top
      ORDER BY students DESC, t.course, t.lesson, t.activity_type
    `);
  }
  return priceStmt;
}
...
/**
 * Columns whose authored price disagrees with what students were served.
 *
 * @param {{force?: boolean, limit?: number}} [opts] force skips the cache.
 * @returns {object|null} null when it could not be measured, never throws.
 */
function priceIntegrity(opts) {
  const force = !!(opts && opts.force);
  const limit = (opts && opts.limit) || 20;
  const now = Date.now();
  if (!force && priceCache && now - priceCachedAt < PRICE_TTL_MS) return priceCache;

  try {
    const rows = priceStatement().all();
    let students = 0;
    for (const r of rows) students += r.students;
    priceCache = {
      ok: rows.length === 0,
      columns: rows.length,
      students_affected: students,
      worst: rows.slice(0, limit).map((r) => ({
        course: r.course, unit: r.unit, lesson: r.lesson, activity_type: r.activity_type,
        authored: r.authored, observed: r.observed, students: r.students,
      })),
    };
    priceCachedAt = now;
    return priceCache;
  } catch (e) {
    console.error('health-integrity: could not measure price agreement:', e.message);
    return null;
  }
}
...
function resetCache() {
  cache = null; cachedAt = 0;
  reporterCache = null; reporterCachedAt = 0;
  priceCache = null; priceCachedAt = 0;
}

module.exports = { codeSeedIntegrity, reporterIntegrity, priceIntegrity, resetCache,
  TTL_MS, REPORTER_TTL_MS, PRICE_TTL_MS, LESSON_NATIVES };
