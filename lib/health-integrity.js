...
//
//  ── WHAT IT COMPARES ────────────────────────────────────────────────────────
//  The authored price in course_denominators against the summed max_points per
//  student per activity from the ledger, derived exactly as the gradebook
//  derives points_possible: reserved carriers excluded through scoring.js so
//  this block and the gradebook cannot hold two opinions about the same column.
//
//  A disagreement is not automatically the price being wrong. A page rebuilt
//  today is served to a student tomorrow, so the honest reading is "these two
//  numbers should be reconciled", which is why the row carries both.
//
//  ── WHY IT IS SAFE TO MAKE PUBLIC ───────────────────────────────────────────
...
function priceStatement() {
  if (!priceStmt) {
    priceStmt = db.prepare(`
      SELECT o.course, o.unit, o.lesson, o.activity_type,
             d.possible        AS authored,
             o.possible        AS observed,
             COUNT(*)          AS students
      FROM (
        SELECT course, unit, lesson, activity_type, student_id, SUM(item_max) AS possible
        FROM (
          SELECT course, unit, lesson, activity_type, student_id, item,
                 MAX(max_points) AS item_max,
                 ${namedItemFlagSql('item', 'course, unit, lesson, activity_type, student_id')} AS has_named
          FROM score_events
          WHERE item <> '${LESSON_SCORE_ITEM}'
          GROUP BY course, unit, lesson, activity_type, student_id, item
        ) WHERE ${keepItemSql('item', 'has_named')}
        GROUP BY course, unit, lesson, activity_type, student_id
      ) o
      JOIN course_denominators d
        ON d.course = o.course AND d.lesson = o.lesson AND d.activity_type = o.activity_type
      WHERE o.possible > 0
        AND o.activity_type NOT IN ('lesson', 'visit', 'page', 'read')
        AND d.possible <> o.possible
      GROUP BY o.course, o.unit, o.lesson, o.activity_type, d.possible, o.possible
      ORDER BY students DESC, o.course, o.lesson, o.activity_type
    `);
  }
  return priceStmt;
}
