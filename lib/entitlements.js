...
// The five paid courses. A 'solo' class (ME- accounts) is intentionally not an
...
//
// intro-java is a paid course in its own right, not a free sampler bundled with
// ap-csa. A teacher can buy the on-ramp without buying CSA (that is most of its
// market: pre-AP and intro classes that never sit the exam), so it needs its own
// entitlement row rather than riding on the CSA grant.
const VALID_COURSES = ['ap-csp', 'ap-csa', 'ap-cybersecurity', 'ap-networking', 'intro-java'];
...
// ── STUDENT-FACING RESOLVER (task #80) ────────────────────────────────────────
//  GET /api/student/entitlement (routes/student.js) answers one question for the
//  site-wide theme script: what has this student's teacher paid for? The route
//  is a thin wrapper; the facts are resolved here so the ad gate, the content
//  gate and the admin views all read the same definition of "paid".
//
//  Unit numbers come from utils.js COURSES, which is the structure authority for
//  every course (CSA units 1-4, CSP big ideas 1-5, Cyber units 1-5). They are
//  computed ONCE at module load into a frozen map: this route fires on every
//  page view of the storefront, so it must not run a query per call, and a
//  precomputed constant cannot go stale the way a TTL cache can.
const { COURSES } = require('../utils');
const COURSE_UNITS = Object.freeze(
  Object.fromEntries(
    Object.entries(COURSES).map(([course, cfg]) => [
      course,
      Object.freeze(
        Object.keys((cfg && cfg.units) || {})
          // 'unit-3' and 'bi-3' both mean unit 3. The theme script keys CSP
          // pages off the big-idea number, so the two must normalize alike.
          .map((k) => parseInt(String(k).replace(/^\D+/, ''), 10))
...
function unitsForCourse(course) { return COURSE_UNITS[course] || []; }

// What is true about this student's access, before any policy is applied.
// Reads the class and teacher fresh from the database, never from the token.
//
//   course    the class's course ('solo' for ME- accounts)
//   enrolled  the student sits in a real teacher's class (not solo)
//   entitled  that class's teacher holds a LIVE entitlement for that course
//
// Solo students have no teacher and therefore no paid seat: enrolled false,
// entitled false. That is the intended posture, matching evaluateStudentGate.
function resolveStudentAccess(studentId) {
  const row = getStudentClassStmt.get(studentId);
  if (!row) return { course: null, enrolled: false, entitled: false };
  const course = row.class_course;
  const enrolled = isValidCourse(course);
  const entitled = enrolled && hasActiveEntitlement(row.teacher_id, course);
  return { course, enrolled, entitled };
}
