...
const METRICS = {
  ...
  //  THREE DIFFERENT DENOMINATORS, and the vendors do not agree on which one a
  //  bare "RPM" means. Keeping them as one metric is how a page rate and a
  //  session rate end up in the same column, and lib/class-monetization.js
  //  multiplies `rpm` by PAGEVIEWS, so getting it wrong is a silent overstatement
  //  by pages-per-session. Measured on Raptive's own export, 2026-09-19: page
  //  $7.20 against session $21.42, a factor of 2.97.
  //
  //  `rpm` is the PAGE rate and nothing else. The label says so, because "RPM"
  //  on a chart axis is exactly the ambiguity this split exists to remove.
  rpm: { unit: 'usd', agg: 'avg', label: 'Page RPM' },
  session_rpm: { unit: 'usd', agg: 'avg', label: 'Session RPM' },
  impression_rpm: { unit: 'usd', agg: 'avg', label: 'Impression RPM (eCPM)' },
};
