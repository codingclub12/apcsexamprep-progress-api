...
//  Use textKeepingLines() wherever the arrangement of the output is the answer.
function text(html) {
  return decode(String(html == null ? '' : html).replace(/<[^>]+>/g, ' ')).replace(/\s+/g, ' ').trim();
}

//  Same, but <br> becomes a newline and existing newlines survive, because in a
//  monospaced option block the line breaks ARE the answer.
function textKeepingLines(html) {
  return decode(String(html == null ? '' : html)
    .replace(/<br\s*\/?>/gi, '\n')
    .replace(/<\/(p|div|li|tr)>/gi, '\n')
    .replace(/<[^>]+>/g, ''));
}
//  LINE STRUCTURE IS NOT WHITESPACE NOISE, and collapsing it invented a defect.
//
...
//  The first version of this collapsed every run of whitespace, so
//
//      "*\n* *\n* * *"   and   "* * *\n* *\n*"
//
//  both became "* * * * * *": the same six stars in a different arrangement,
//  reported as two identical options on a page where they are the whole point
//  of the question. A false accusation sends somebody to rewrite a correct item.
//
//  So a newline is significant and a run of spaces inside a line is not. Each
//  line is trimmed, its internal spacing collapsed, and trailing blank lines
//  dropped, which is what a reader sees in a monospaced block.
...
//  TWO COMPARISONS, BECAUSE TWO THINGS ARE BEING COMPARED.
//
//  norm() flattens lines. It is for prose that RESTATES an answer: the
//  <h3>Answer: (A) ...</h3> heading is one line of markup and cannot carry the
//  option's line breaks, so holding it to them reported eight correct pages as
//  mismatched, where the only difference was that the option is three lines and
//  the heading says the same thing on one.
//
//  normLines() keeps them. It is for comparing an option against another option
//  or against what the program printed, where the arrangement IS the answer:
//  a growing triangle and a shrinking one are the same six stars.
//
//  Using either one for both jobs invents findings, and this file has now done
//  it in both directions, one after the other.
function norm(s) {
  return textKeepingLines(s).replace(/\s+/g, ' ').trim().toLowerCase().replace(/[.,;:!?"'`]+/g, '');
}

function normLines(s) {
  return collapseLines(textKeepingLines(s)).toLowerCase().replace(/[.,;:!?"'`]+/g, '');
}
...
function readOptions(clean) {
  ...
  while ((m = re.exec(clean))) {
    ...
    if (practice) { out.push({ letter, text: collapseLines(textKeepingLines(practice[1])), via: 'apcs-option-content' }); continue; }
    ...
    if (spans.length) { out.push({ letter, text: collapseLines(textKeepingLines(spans[spans.length - 1][1])), via: 'span' }); continue; }

    //  Neither shape. Report the raw text and say so rather than inventing a rule.
    const bare = collapseLines(textKeepingLines(frag));
...
module.exports = { parse, decode, text, textKeepingLines, collapseLines, norm, normLines, LETTERS, KEY_VARS, withoutStyle };
