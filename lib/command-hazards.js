...
and breaks the script. (Mojibake is what an unescaped character produces: read as
cp1252, a bullet's UTF-8 bytes become the THREE characters U+00E2 U+20AC U+00A2 and
an emoji's become FOUR. \`lib/mojibake.js\` detects both by reversing them, and
\`npm run smoke:encoding\` is the gate. Do not paste a corrupted example into a rule:
the drafts for the 2026-09-03 fix carried a DOUBLY corrupted sample, and a rule
matching it is blind to the single-pass form that actually reaches a page.)
