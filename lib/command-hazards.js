...
and breaks the script. (\`•\` / \`ðŸŽ¯\` mojibake is what unescaped characters produce.)
...
// AP Networking. Composed from the parts of the other four blocks that are NOT
// curriculum: the cross-course production rules and the zero-PII posture. Those
// hold for this course because they hold for every course in this product.
//
// The curriculum half is deliberately absent and says so in its first line. No
// source-of-truth filename is guessed here on purpose: a plausible-looking PDF
// name injected verbatim into every future prompt is worse than an admitted gap,
// because an agent will cite it and nobody will check whether it exists.
const CONTENT_NETWORKING = `AP Networking. THE CURRICULUM SPECIFICS FOR THIS COURSE ARE NOT RECORDED HERE YET: no
source-of-truth document, no unit or lesson structure, no titles, no notation or terminology
conventions. Do not author or renumber lesson content until they are added, and do not infer
them from AP Cybersecurity or from general networking knowledge. A wrong lesson title or a
wrong layer model, injected verbatim into every future prompt, is worse than the gap it fills.
Ask Tanner, add them here, then recompile this prompt.

Everything below is NOT curriculum. It holds for this course because it holds for every course
in this product, so it applies to any networking work done meanwhile.

Never import another course's unit numbering or structure. AP CSA is 4 units, Intro to Java is
6, AP Cybersecurity has its own; none of them are this course's, and borrowing one to fill the
gap above is the specific mistake this block exists to prevent.

Speaker notes stay in student-addressed voice: the same notes are the single source for BOTH
the teacher bundle and the video narration, so a teacher-directed instruction written into them
has to be removed twice. Teacher Bundle decks carry no prescriptive teacher instructions
(timing banners, cold-call directives, "yesterday/tomorrow" pacing language).
...
indices and booleans only. This is not a preference, it is the zero-PII posture the whole
product rests on, and "just for diagnostics" is how it gets broken.`;
...
const CONTENT_COVERAGE = {
  ...
  networking: {
    title: 'AP Networking content (curriculum specifics NOT recorded)',
    block: CONTENT_NETWORKING,
    review: 'Shipped. The cross-course production rules and the zero-PII posture are enforced, '
      + 'but no source-of-truth document, unit structure, lesson titles or notation conventions '
      + 'are recorded. Those must come from Tanner and must not be inferred. Until they are '
      + 'here, the block stops lesson authoring rather than the whole task.',
  },
  'intro-java': {
