...
//
//  NO TEACHER-FACING CITATIONS IN STUDENT-FACING FIELDS
//  The bundle stems cite the framework inline: "Per EK 1.1.A.2, which two
//  tactics...", and 1.2 Q9 used bare EK codes as its four answer choices. That
//  reads correctly in a document a teacher grades from and wrongly on a page a
//  student sits. It also hands over the search term that finds the answer.
//
//  So every prompt and every option here is written in student register, with no
//  EK, LO, CED or CB reference. Only the wording changed: qids, option order,
//  correct_index and points are exactly what the answer grids above say, which is
//  why this could not disturb a gradebook. smoke/quiz-bank-authoring.js enforces
//  the rule, so the 25 Unit 1 quiz pages still unaudited inherit it rather than
//  each needing the same edit found again by hand.
//
//  Explanations DO still cite the EK, deliberately. They ship only after a
//  teacher releases the key (routes/quiz.js gates them on key_releases), and at
//  that point naming the EK is the useful thing to do: it tells a student who got
//  it wrong exactly where to look.
//
//  OPEN, AND NOT DECIDED HERE: whether the online quiz should stay a faithful
//  copy of the bundle instrument at all. Matching it is what the reporting
//  teacher asked for, and it is also what makes a leaked teacher document a
//  leaked online assessment. Diverging means authoring a second pool against the
//  same EKs, which is content work and a pricing question, not a code change.
//
...
const L11 = [
  {
    prompt: 'A student gets a text: "This is IT Support. Your account will be permanently deleted in 5 minutes unless you reply with the 6-digit code we just sent." Which two tactics does this message use?',
    options: [
      'Intimidation, the threat of permanent deletion, and urgency, the 5-minute deadline',
      'Reciprocity and authority bias',
  ...
  {
    prompt: 'Social engineering attacks use psychological tactics to manipulate users into doing which of the following?',
    options: [
  ...
  {
    prompt: "Through which channels can a social engineering attack arrive?",
    options: [
  ...
  {
    prompt: 'A message warns, "If you do not verify now, your paycheck will be withheld." Explain how intimidation works on the reader.',
    options: [
  ...
  {
    prompt: 'Why is a sense of urgency effective at getting a target to comply?',
    options: [
  ...
  {
    prompt: 'What do social engineering tactics fundamentally rely on?',
    options: [
  ...
  {
    prompt: "A victim is tricked into revealing a pet's name, birthdate, and former workplace. Why is this information valuable to an adversary?",
    options: [
  ...
  {
    prompt: 'An adversary convinces a victim to read back the one-time password (OTP) texted to their phone. What is the direct impact?',
    options: [
  ...
  {
    prompt: 'Clicking a malicious link or downloading malware can lead to which outcomes? I. Malware installed on the device. II. Information stolen from the web browser. III. Being directed to a site that captures login credentials.',
    options: [
...
const L12 = [
  ...
  {
    prompt: 'Which password most directly reflects the common pattern of one or two words plus a two-digit year plus a special character at the end?',
  ...
  {
    prompt: 'A review of one account\'s sign-in history flags two suspicious sign-ins on school days from an unknown IP address. A student recommends: "Change the account password immediately." Which recommended control is MISSING from this response?',
    options: [
  ...
  {
    prompt: 'What does an adversary attempt during an online password attack?',
    options: [
  ...
  {
    prompt: "A help-desk log shows a user's account with steady, successful logins every weekday at 8 a.m. from one office laptop. Overnight, the same account logs 3,000 failed attempts in 20 minutes from an IP in another country. Which sign of an online password attack is MOST clearly evidenced by the overnight activity?",
    options: [
  ...
  {
    prompt: 'A user wants a password that is long, random, and unique, and that avoids personally meaningful information. Which choice best satisfies both goals?',
    options: [
  ...
  {
    prompt: "An adversary scrolls a target's public profiles and records the names of their two children, their wedding anniversary, and their cat's name. They feed these into a tool that auto-submits combinations against the login form. Which method best describes this attack?",
    options: [
      'Trying stolen passwords leaked from an unrelated breach',
      'Building a dictionary of likely passwords from gathered personal information',
      'Enabling multifactor authentication on the account',
      'Logging in successfully from a known device',
    ],
    correct_index: 1,
  ...
  {
    prompt: "An account's history shows logins only from a home laptop on weekday evenings. A security review flags four new events on the same account. Which single event most clearly matches the 'login attempts from unknown devices' sign?",
    options: [
  ...
  {
    prompt: 'Which choice is NOT one of the common patterns people use when creating passwords?',
    options: [
  ...
  {
    prompt: "A user's password is stolen in a breach. With multifactor authentication enabled on the account, the adversary types the correct password into the login form. What happens next?",
    options: [
