...
'use strict';
// -----------------------------------------------------------------------------
//  BUILD THE MATRIXIFY SHEET FOR THE TOPIC 1.4 QUIZ.
//
//    node scripts/cyber-u1-l4-quiz-ced-csv.js out/l4quiz.csv [--live page.json]
//                                             [--show-changes] [--html f.html]
//
//  Quiz-shaped checks live in lib/cyber-quiz-gate.js and page-level ones in
//  lib/cyber-page-gate.js. This file is the wiring.
// -----------------------------------------------------------------------------

const fs = require('fs');
const path = require('path');
const mod = require('../lib/cyber-u1-l4-quiz-ced');
const gate0 = require('../lib/cyber-page-gate');
const quizgate = require('../lib/cyber-quiz-gate');
...
function gate(before, after) {
  //  namesLegacyTerm is shared with the exercises on purpose: the same word is
  //  taught as vocabulary or used as ordinary English on either page shape, and
  //  the distinction does not change because the widget did.
  const { fail, note } = quizgate.check(before, after, exgate.namesLegacyTerm);

  fail.push(...gate0.nothingUnhidden(before, after));
  fail.push(...gate0.balancedTags(after, ['div', 'style', 'script', 'span']));
  fail.push(...gate0.scriptsParse(after));
  fail.push(...gate0.noNewNonAscii(before, after));

  const changed = gate0.changedSentences(before, after, quizgate.flat);
...
async function main() {
  ...
  if (!out) {
    console.error('usage: node scripts/cyber-u1-l4-quiz-ced-csv.js <out.csv> [--live f.json] [--show-changes] [--html f.html]');
    process.exit(2);
  }
  ...
  if (!show) {
    console.error('\nRefusing to write without --show-changes. This touches a graded quiz;');
    console.error('a passing gate says the keys are reachable, not that they are the right keys.');
    process.exit(1);
  }
