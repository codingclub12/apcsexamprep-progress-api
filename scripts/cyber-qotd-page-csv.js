...
//  Four of the page's 24 topic titles disagree with the CED taxonomy in
//  config/cyber-topics.json (1.2, 2.1, 3.2, 5.2). Two are truncations and two
//  are different titles outright. They are REPORTED, not silently rewritten:
//  topic 1.2 is the subject of board task 170, and quietly changing a contested
//  label inside an unrelated sheet is how a taxonomy drifts. Run with --topics
//  to print the diff.
//
//  Usage:
...
function bankHtml() {
  ...
  for (const [unit, topics] of [...units.entries()].sort((a, b) => a[0] - b[0])) {
    ...
    for (const [topic, qs] of [...topics.entries()].sort((a, b) => parseFloat(a[0]) - parseFloat(b[0]))) {
      out.push(`      <h4 class="cy-bank-th">Topic ${esc(topic)} ${esc(POOL.TOPIC_TITLES[topic])}</h4>`);
      for (const q of qs) {
...
function unitPageBody(unit, qs) {
  ...
  for (const [topic, list] of [...topics.entries()].sort((a, b) => parseFloat(a[0]) - parseFloat(b[0]))) {
    parts.push(`      <h2 class="cy-bank-th">Topic ${esc(topic)} ${esc(POOL.TOPIC_TITLES[topic])}</h2>`);
...
function main() {
  ...
  const t = topicDiff();
  if (t.length) {
    console.log(`NOTE: ${t.length} topic titles differ from the CED taxonomy (run --topics). `
      + 'Not rewritten here on purpose; topic 1.2 is board task 170.');
  }
}
